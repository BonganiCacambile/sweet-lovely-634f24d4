// @ts-nocheck
/* eslint-disable */
/* This file was generated by Unframer for Framer project a377177818209678 "Pepper (copy)", do not edit manually */
"use client";

/**
 * @typedef Locale
 * string
 */

/**
 * @typedef {{
 * children?: React.ReactNode
 * locale?: Locale
 * style?: React.CSSProperties
 * className?: string
 * id?: string
 * ref?: any
 * width?: any
 * height?: any
 * layoutId?: string
 * variant?: 'Spicy Duo Deal' | 'Cheese Lovers Pair' | 'Veggie Delight Duo' | 'Sweet & Savory Combo' | 'Meat Feast Combo' | 'Meat Feast Combo - Big' | 'Spicy Duo Deal - Mobile' | 'Cheese Lovers Pair - Mobile' | 'Veggie Delight Duo - Mobile' | 'Sweet & Savory Combo - Mobile' | 'Meat Feast Combo - Mobile' | 'Meat Feast Combo - Mobile - Tablet' // Variant
}} Props

 */
import { className, css, fonts } from "../chunks/chunk-2XRBDIEF.js";
import {
	className as className3,
	css as css3,
	fonts as fonts3,
} from "../chunks/chunk-TEXBX6PP.js";
import { stdin_default } from "../chunks/chunk-RGEGD6RN.js";
import {
	className as className2,
	css as css2,
	fonts as fonts2,
} from "../chunks/chunk-BW6CAMPU.js";
import { routes } from "../chunks/chunk-H6F5WHVF.js";

// virtual:menu-products/offer-crad
import { Fragment as Fragment2 } from "react";
import { ContextProviders } from "unframer";

// /:https://framerusercontent.com/modules/pnF1PIP9Z7Xyk1a22RvS/PFMJcPlHqgSyurqUU0TD/sdO1Bzv_P.js
import {
	jsx as _jsx,
	jsxs as _jsxs,
	Fragment as _Fragment,
} from "react/jsx-runtime";
import {
	addFonts,
	addPropertyControls,
	ChildrenCanSuspend,
	ComponentViewportProvider,
	ControlType,
	cx,
	forwardLoader,
	getFonts,
	getFontsFromSharedStyle,
	Image,
	PathVariablesContext,
	queryCache,
	RichText,
	SmartComponentScopedContainer,
	useComponentViewport,
	useLocaleInfo,
	useQueryData,
	useVariantState,
	withCSS,
	withFX,
} from "unframer";
import { LayoutGroup, motion, MotionConfigContext } from "unframer";
import * as React from "react";
import { useRef } from "react";

// /:https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js
import {
	addPropertyControls as e5,
	ControlType as l3,
	QueryEngine as t4,
} from "unframer";

// /:https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch-0.js
import { ControlType as y } from "unframer";
import { ControlType as P } from "unframer";
var __unframerWindow = typeof window !== "undefined" ? window : void 0;
var t;
var e = Object.create;
var r = Object.defineProperty;
var n = Object.getOwnPropertyDescriptor;
var i = Object.getOwnPropertyNames;
var s = Object.getPrototypeOf;
var a = Object.prototype.hasOwnProperty;
var o = (t32, e42, n22) =>
	e42 in t32
		? r(t32, e42, {
				enumerable: true,
				configurable: true,
				writable: true,
				value: n22,
			})
		: (t32[e42] = n22);
var u = (t32, e42) =>
	function () {
		return (
			e42 ||
				(0, t32[i(t32)[0]])(
					(e42 = {
						exports: {},
					}).exports,
					e42,
				),
			e42.exports
		);
	};
var l = (t32, e42, s22, o22) => {
	if ((e42 && "object" == typeof e42) || "function" == typeof e42)
		for (let u22 of i(e42))
			a.call(t32, u22) ||
				u22 === s22 ||
				r(t32, u22, {
					get: () => e42[u22],
					enumerable: !(o22 = n(e42, u22)) || o22.enumerable,
				});
	return t32;
};
var h = (t32, n22, i22) => (
	(i22 = null != t32 ? e(s(t32)) : {}),
	l(
		// If the importer is in node compatibility mode or this is not an ESM
		// file that has been converted to a CommonJS file using a Babel-
		// compatible transform (i.e. "__esModule" has not been set), then set
		// "default" to the CommonJS "module.exports" for node compatibility.
		!n22 && t32 && t32.__esModule
			? i22
			: r(i22, "default", {
					value: t32,
					enumerable: true,
				}),
		t32,
	)
);
var c = (t32, e42, r22) => o(t32, "symbol" != typeof e42 ? e42 + "" : e42, r22);
var f = u({
	"../../../node_modules/dataloader/index.js"(t32, e42) {
		var r22,
			n22 = /* @__PURE__ */ (function () {
				function t42(t5, e6) {
					if ("function" != typeof t5)
						throw TypeError(
							"DataLoader must be constructed with a function which accepts Array<key> and returns Promise<Array<value>>, but got: " +
								t5 +
								".",
						);
					(this._batchLoadFn = t5),
						(this._maxBatchSize = (function (t6) {
							if (!(!t6 || false !== t6.batch)) return 1;
							var e7 = t6 && t6.maxBatchSize;
							if (void 0 === e7) return 1 / 0;
							if ("number" != typeof e7 || e7 < 1)
								throw TypeError(
									"maxBatchSize must be a positive number: " + e7,
								);
							return e7;
						})(e6)),
						(this._batchScheduleFn = (function (t6) {
							var e7 = t6 && t6.batchScheduleFn;
							if (void 0 === e7) return i22;
							if ("function" != typeof e7)
								throw TypeError("batchScheduleFn must be a function: " + e7);
							return e7;
						})(e6)),
						(this._cacheKeyFn = (function (t6) {
							var e7 = t6 && t6.cacheKeyFn;
							if (void 0 === e7)
								return function (t7) {
									return t7;
								};
							if ("function" != typeof e7)
								throw TypeError("cacheKeyFn must be a function: " + e7);
							return e7;
						})(e6)),
						(this._cacheMap = (function (t6) {
							if (!(!t6 || false !== t6.cache)) return null;
							var e7 = t6 && t6.cacheMap;
							if (void 0 === e7) return /* @__PURE__ */ new Map();
							if (null !== e7) {
								var r3 = ["get", "set", "delete", "clear"].filter(
									function (t7) {
										return e7 && "function" != typeof e7[t7];
									},
								);
								if (0 !== r3.length)
									throw TypeError(
										"Custom cacheMap missing methods: " + r3.join(", "),
									);
							}
							return e7;
						})(e6)),
						(this._batch = null),
						(this.name = e6 && e6.name ? e6.name : null);
				}
				var e52 = t42.prototype;
				return (
					(e52.load = function (t5) {
						if (null == t5)
							throw TypeError(
								"The loader.load() function must be called with a value, but got: " +
									String(t5) +
									".",
							);
						var e6 = (function (t6) {
								var e7 = t6._batch;
								if (
									null !== e7 &&
									!e7.hasDispatched &&
									e7.keys.length < t6._maxBatchSize
								)
									return e7;
								var r4 = {
									hasDispatched: false,
									keys: [],
									callbacks: [],
								};
								return (
									(t6._batch = r4),
									t6._batchScheduleFn(function () {
										(function (t7, e8) {
											var r5;
											if (((e8.hasDispatched = true), 0 === e8.keys.length)) {
												a22(e8);
												return;
											}
											try {
												r5 = t7._batchLoadFn(e8.keys);
											} catch (r6) {
												return s22(
													t7,
													e8,
													TypeError(
														"DataLoader must be constructed with a function which accepts Array<key> and returns Promise<Array<value>>, but the function errored synchronously: " +
															String(r6) +
															".",
													),
												);
											}
											if (!r5 || "function" != typeof r5.then)
												return s22(
													t7,
													e8,
													TypeError(
														"DataLoader must be constructed with a function which accepts Array<key> and returns Promise<Array<value>>, but the function did not return a Promise: " +
															String(r5) +
															".",
													),
												);
											r5.then(function (t8) {
												if (!o22(t8))
													throw TypeError(
														"DataLoader must be constructed with a function which accepts Array<key> and returns Promise<Array<value>>, but the function did not return a Promise of an Array: " +
															String(t8) +
															".",
													);
												if (t8.length !== e8.keys.length)
													throw TypeError(
														"DataLoader must be constructed with a function which accepts Array<key> and returns Promise<Array<value>>, but the function did not return a Promise of an Array of the same length as the Array of keys.\n\nKeys:\n" +
															String(e8.keys) +
															"\n\nValues:\n" +
															String(t8),
													);
												a22(e8);
												for (var r6 = 0; r6 < e8.callbacks.length; r6++) {
													var n4 = t8[r6];
													n4 instanceof Error
														? e8.callbacks[r6].reject(n4)
														: e8.callbacks[r6].resolve(n4);
												}
											}).catch(function (r6) {
												s22(t7, e8, r6);
											});
										})(t6, r4);
									}),
									r4
								);
							})(this),
							r3 = this._cacheMap,
							n32 = this._cacheKeyFn(t5);
						if (r3) {
							var i32 = r3.get(n32);
							if (i32) {
								var u22 = e6.cacheHits || (e6.cacheHits = []);
								return new Promise(function (t6) {
									u22.push(function () {
										t6(i32);
									});
								});
							}
						}
						e6.keys.push(t5);
						var l22 = new Promise(function (t6, r4) {
							e6.callbacks.push({
								resolve: t6,
								reject: r4,
							});
						});
						return r3 && r3.set(n32, l22), l22;
					}),
					(e52.loadMany = function (t5) {
						if (!o22(t5))
							throw TypeError(
								"The loader.loadMany() function must be called with Array<key> but got: " +
									t5 +
									".",
							);
						for (var e6 = [], r3 = 0; r3 < t5.length; r3++)
							e6.push(
								this.load(t5[r3]).catch(function (t6) {
									return t6;
								}),
							);
						return Promise.all(e6);
					}),
					(e52.clear = function (t5) {
						var e6 = this._cacheMap;
						if (e6) {
							var r3 = this._cacheKeyFn(t5);
							e6.delete(r3);
						}
						return this;
					}),
					(e52.clearAll = function () {
						var t5 = this._cacheMap;
						return t5 && t5.clear(), this;
					}),
					(e52.prime = function (t5, e6) {
						var r3 = this._cacheMap;
						if (r3) {
							var n32,
								i32 = this._cacheKeyFn(t5);
							void 0 === r3.get(i32) &&
								(e6 instanceof Error
									? (n32 = Promise.reject(e6)).catch(function () {})
									: (n32 = Promise.resolve(e6)),
								r3.set(i32, n32));
						}
						return this;
					}),
					t42
				);
			})(),
			i22 =
				"object" == typeof process && "function" == typeof process.nextTick
					? function (t42) {
							r22 || (r22 = Promise.resolve()),
								r22.then(function () {
									process.nextTick(t42);
								});
						}
					: "function" == typeof setImmediate
						? function (t42) {
								setImmediate(t42);
							}
						: function (t42) {
								setTimeout(t42);
							};
		function s22(t42, e52, r3) {
			a22(e52);
			for (var n32 = 0; n32 < e52.keys.length; n32++)
				t42.clear(e52.keys[n32]), e52.callbacks[n32].reject(r3);
		}
		function a22(t42) {
			if (t42.cacheHits)
				for (var e52 = 0; e52 < t42.cacheHits.length; e52++)
					t42.cacheHits[e52]();
		}
		function o22(t42) {
			return (
				"object" == typeof t42 &&
				null !== t42 &&
				"number" == typeof t42.length &&
				(0 === t42.length ||
					(t42.length > 0 &&
						Object.prototype.hasOwnProperty.call(t42, t42.length - 1)))
			);
		}
		e42.exports = n22;
	},
});
var d = h(f(), 1);
var g = {
	Uint8: 1,
	Uint16: 2,
	Uint32: 4,
	BigUint64: 8,
	Int8: 1,
	Int16: 2,
	Int32: 4,
	BigInt64: 8,
	Float32: 4,
	Float64: 8,
};
var p = class {
	getOffset() {
		return this.offset;
	}
	ensureLength(t32) {
		let e42 = this.bytes.length;
		if (!(this.offset + t32 <= e42)) throw Error("Reading out of bounds");
	}
	readUint8() {
		let t32 = g.Uint8;
		this.ensureLength(t32);
		let e42 = this.view.getUint8(this.offset);
		return (this.offset += t32), e42;
	}
	readUint16() {
		let t32 = g.Uint16;
		this.ensureLength(t32);
		let e42 = this.view.getUint16(this.offset);
		return (this.offset += t32), e42;
	}
	readUint32() {
		let t32 = g.Uint32;
		this.ensureLength(t32);
		let e42 = this.view.getUint32(this.offset);
		return (this.offset += t32), e42;
	}
	readUint64() {
		let t32 = this.readBigUint64();
		return Number(t32);
	}
	readBigUint64() {
		let t32 = g.BigUint64;
		this.ensureLength(t32);
		let e42 = this.view.getBigUint64(this.offset);
		return (this.offset += t32), e42;
	}
	readInt8() {
		let t32 = g.Int8;
		this.ensureLength(t32);
		let e42 = this.view.getInt8(this.offset);
		return (this.offset += t32), e42;
	}
	readInt16() {
		let t32 = g.Int16;
		this.ensureLength(t32);
		let e42 = this.view.getInt16(this.offset);
		return (this.offset += t32), e42;
	}
	readInt32() {
		let t32 = g.Int32;
		this.ensureLength(t32);
		let e42 = this.view.getInt32(this.offset);
		return (this.offset += t32), e42;
	}
	readInt64() {
		let t32 = this.readBigInt64();
		return Number(t32);
	}
	readBigInt64() {
		let t32 = g.BigInt64;
		this.ensureLength(t32);
		let e42 = this.view.getBigInt64(this.offset);
		return (this.offset += t32), e42;
	}
	readFloat32() {
		let t32 = g.Float32;
		this.ensureLength(t32);
		let e42 = this.view.getFloat32(this.offset);
		return (this.offset += t32), e42;
	}
	readFloat64() {
		let t32 = g.Float64;
		this.ensureLength(t32);
		let e42 = this.view.getFloat64(this.offset);
		return (this.offset += t32), e42;
	}
	readBytes(t32) {
		let e42 = this.offset,
			r22 = e42 + t32,
			n22 = this.bytes.subarray(e42, r22);
		return (this.offset = r22), n22;
	}
	readString() {
		let t32 = this.readUint32(),
			e42 = this.readBytes(t32);
		return this.decoder.decode(e42);
	}
	readJson() {
		let t32 = this.readString();
		return JSON.parse(t32);
	}
	constructor(t32) {
		(this.bytes = t32),
			c(this, "offset", 0),
			c(this, "view"),
			c(this, "decoder", new TextDecoder()),
			(this.view = v(this.bytes));
	}
};
function v(t32) {
	return new DataView(t32.buffer, t32.byteOffset, t32.byteLength);
}
var m = "undefined" != typeof __unframerWindow;
var w = m && "function" == typeof __unframerWindow.requestIdleCallback;
function I(t32, ...e42) {
	if (!t32)
		throw Error(
			"Assertion Error" + (e42.length > 0 ? ": " + e42.join(" ") : ""),
		);
}
function b(t32) {
	throw Error(`Unexpected value: ${t32}`);
}
var U = 1024;
var S = 1.5;
var k = (t32) => 2 ** t32 - 1;
var L = (t32) => -(2 ** (t32 - 1));
var B = (t32) => 2 ** (t32 - 1) - 1;
var E = {
	Uint8: 0,
	Uint16: 0,
	Uint32: 0,
	Uint64: 0,
	BigUint64: 0,
	Int8: L(8),
	Int16: L(16),
	Int32: L(32),
	Int64: Number.MIN_SAFE_INTEGER,
	BigInt64: -(BigInt(2) ** BigInt(63)),
};
var M = {
	Uint8: k(8),
	Uint16: k(16),
	Uint32: k(32),
	Uint64: Number.MAX_SAFE_INTEGER,
	BigUint64: BigInt(2) ** BigInt(64) - BigInt(1),
	Int8: B(8),
	Int16: B(16),
	Int32: B(32),
	Int64: Number.MAX_SAFE_INTEGER,
	BigInt64: BigInt(2) ** BigInt(63) - BigInt(1),
};
function T(t32, e42, r22, n22) {
	I(t32 >= e42, t32, "outside lower bound for", n22),
		I(t32 <= r22, t32, "outside upper bound for", n22);
}
var F = class {
	getOffset() {
		return this.offset;
	}
	slice(t32 = 0, e42 = this.offset) {
		return this.bytes.slice(t32, e42);
	}
	subarray(t32 = 0, e42 = this.offset) {
		return this.bytes.subarray(t32, e42);
	}
	ensureLength(t32) {
		let e42 = this.bytes.length;
		if (this.offset + t32 <= e42) return;
		let r22 = new Uint8Array(Math.ceil(e42 * S) + t32);
		r22.set(this.bytes), (this.bytes = r22), (this.view = v(r22));
	}
	writeUint8(t32) {
		T(t32, E.Uint8, M.Uint8, "Uint8");
		let e42 = g.Uint8;
		this.ensureLength(e42),
			this.view.setUint8(this.offset, t32),
			(this.offset += e42);
	}
	writeUint16(t32) {
		T(t32, E.Uint16, M.Uint16, "Uint16");
		let e42 = g.Uint16;
		this.ensureLength(e42),
			this.view.setUint16(this.offset, t32),
			(this.offset += e42);
	}
	writeUint32(t32) {
		T(t32, E.Uint32, M.Uint32, "Uint32");
		let e42 = g.Uint32;
		this.ensureLength(e42),
			this.view.setUint32(this.offset, t32),
			(this.offset += e42);
	}
	writeUint64(t32) {
		T(t32, E.Uint64, M.Uint64, "Uint64");
		let e42 = BigInt(t32);
		this.writeBigUint64(e42);
	}
	writeBigUint64(t32) {
		T(t32, E.BigUint64, M.BigUint64, "BigUint64");
		let e42 = g.BigUint64;
		this.ensureLength(e42),
			this.view.setBigUint64(this.offset, t32),
			(this.offset += e42);
	}
	writeInt8(t32) {
		T(t32, E.Int8, M.Int8, "Int8");
		let e42 = g.Int8;
		this.ensureLength(e42),
			this.view.setInt8(this.offset, t32),
			(this.offset += e42);
	}
	writeInt16(t32) {
		T(t32, E.Int16, M.Int16, "Int16");
		let e42 = g.Int16;
		this.ensureLength(e42),
			this.view.setInt16(this.offset, t32),
			(this.offset += e42);
	}
	writeInt32(t32) {
		T(t32, E.Int32, M.Int32, "Int32");
		let e42 = g.Int32;
		this.ensureLength(e42),
			this.view.setInt32(this.offset, t32),
			(this.offset += e42);
	}
	writeInt64(t32) {
		T(t32, E.Int64, M.Int64, "Int64");
		let e42 = BigInt(t32);
		this.writeBigInt64(e42);
	}
	writeBigInt64(t32) {
		T(t32, E.BigInt64, M.BigInt64, "BigInt64");
		let e42 = g.BigInt64;
		this.ensureLength(e42),
			this.view.setBigInt64(this.offset, t32),
			(this.offset += e42);
	}
	writeFloat32(t32) {
		let e42 = g.Float32;
		this.ensureLength(e42),
			this.view.setFloat32(this.offset, t32),
			(this.offset += e42);
	}
	writeFloat64(t32) {
		let e42 = g.Float64;
		this.ensureLength(e42),
			this.view.setFloat64(this.offset, t32),
			(this.offset += e42);
	}
	writeBytes(t32) {
		let e42 = t32.length;
		this.ensureLength(e42),
			this.bytes.set(t32, this.offset),
			(this.offset += e42);
	}
	encodeString(t32) {
		let e42 = this.encodedStrings.get(t32);
		if (e42) return e42;
		let r22 = this.encoder.encode(t32);
		return this.encodedStrings.set(t32, r22), r22;
	}
	writeString(t32) {
		let e42 = this.encodeString(t32),
			r22 = e42.length;
		this.writeUint32(r22), this.writeBytes(e42);
	}
	writeJson(t32) {
		let e42 = JSON.stringify(t32);
		this.writeString(e42);
	}
	constructor() {
		c(this, "offset", 0),
			c(this, "bytes", new Uint8Array(U)),
			c(this, "view", v(this.bytes)),
			c(this, "encoder", new TextEncoder()),
			c(this, "encodedStrings", /* @__PURE__ */ new Map());
	}
};
function x(t32) {
	return "string" == typeof t32;
}
function N(t32) {
	return Number.isFinite(t32);
}
function A(t32) {
	return null === t32;
}
var O = class t2 {
	static fromString(e42) {
		let [r22, n22, i22] = e42.split("/").map(Number);
		return (
			I(N(r22), "Invalid chunkId"),
			I(N(n22), "Invalid offset"),
			I(N(i22), "Invalid length"),
			new t2(r22, n22, i22)
		);
	}
	toString() {
		return `${this.chunkId}/${this.offset}/${this.length}`;
	}
	static read(e42) {
		let r22 = e42.readUint16(),
			n22 = e42.readUint32(),
			i22 = e42.readUint32();
		return new t2(r22, n22, i22);
	}
	write(t32) {
		t32.writeUint16(this.chunkId),
			t32.writeUint32(this.offset),
			t32.writeUint32(this.length);
	}
	compare(t32) {
		return this.chunkId < t32.chunkId
			? -1
			: this.chunkId > t32.chunkId
				? 1
				: this.offset < t32.offset
					? -1
					: this.offset > t32.offset
						? 1
						: (I(this.length === t32.length), 0);
	}
	constructor(t32, e42, r22) {
		(this.chunkId = t32), (this.offset = e42), (this.length = r22);
	}
};
function R(t32) {
	if (A(t32)) return 0;
	switch (t32.type) {
		case P.Array:
			return 1;
		case P.Boolean:
			return 2;
		case P.Color:
			return 3;
		case P.Date:
			return 4;
		case P.Enum:
			return 5;
		case P.File:
			return 6;
		case P.ResponsiveImage:
			return 10;
		case P.Link:
			return 7;
		case P.Number:
			return 8;
		case P.Object:
			return 9;
		case P.RichText:
			return 11;
		case P.String:
			return 12;
		case P.VectorSetItem:
			return 13;
		default:
			b(t32);
	}
}
function q(e42) {
	let r22 = e42.readUint16(),
		n22 = [];
	for (let i22 = 0; i22 < r22; i22++) {
		let r3 = t.read(e42);
		n22.push(r3);
	}
	return {
		type: P.Array,
		value: n22,
	};
}
function _(e42, r22) {
	for (let n22 of (e42.writeUint16(r22.value.length), r22.value))
		t.write(e42, n22);
}
function D(e42, r22, n22) {
	let i22 = e42.value.length,
		s22 = r22.value.length;
	if (i22 < s22) return -1;
	if (i22 > s22) return 1;
	for (let s32 = 0; s32 < i22; s32++) {
		let i32 = e42.value[s32],
			a22 = r22.value[s32],
			o22 = t.compare(i32, a22, n22);
		if (0 !== o22) return o22;
	}
	return 0;
}
function j(t32) {
	return {
		type: P.Boolean,
		value: 0 !== t32.readUint8(),
	};
}
function C(t32, e42) {
	t32.writeUint8(e42.value ? 1 : 0);
}
function J(t32, e42) {
	return t32.value < e42.value ? -1 : t32.value > e42.value ? 1 : 0;
}
function V(t32) {
	return {
		type: P.Color,
		value: t32.readString(),
	};
}
function W(t32, e42) {
	t32.writeString(e42.value);
}
function $(t32, e42) {
	return t32.value < e42.value ? -1 : t32.value > e42.value ? 1 : 0;
}
function z(t32) {
	let e42 = t32.readInt64(),
		r22 = new Date(e42);
	return {
		type: P.Date,
		value: r22.toISOString(),
	};
}
function G(t32, e42) {
	let r22 = new Date(e42.value),
		n22 = r22.getTime();
	t32.writeInt64(n22);
}
function K(t32, e42) {
	let r22 = new Date(t32.value),
		n22 = new Date(e42.value);
	return r22 < n22 ? -1 : r22 > n22 ? 1 : 0;
}
function H(t32) {
	return {
		type: P.Enum,
		value: t32.readString(),
	};
}
function X(t32, e42) {
	t32.writeString(e42.value);
}
function Q(t32, e42) {
	return t32.value < e42.value ? -1 : t32.value > e42.value ? 1 : 0;
}
function Y(t32) {
	return {
		type: P.File,
		value: t32.readString(),
	};
}
function Z(t32, e42) {
	t32.writeString(e42.value);
}
function tt(t32, e42) {
	return t32.value < e42.value ? -1 : t32.value > e42.value ? 1 : 0;
}
function te(t32) {
	return {
		type: P.Link,
		value: t32.readJson(),
	};
}
function tr(t32, e42) {
	t32.writeJson(e42.value);
}
function tn(t32, e42) {
	let r22 = JSON.stringify(t32.value),
		n22 = JSON.stringify(e42.value);
	return r22 < n22 ? -1 : r22 > n22 ? 1 : 0;
}
function ti(t32) {
	return {
		type: P.Number,
		value: t32.readFloat64(),
	};
}
function ts(t32, e42) {
	t32.writeFloat64(e42.value);
}
function ta(t32, e42) {
	return t32.value < e42.value ? -1 : t32.value > e42.value ? 1 : 0;
}
function to(e42) {
	let r22 = e42.readUint16(),
		n22 = {};
	for (let i22 = 0; i22 < r22; i22++) {
		let r3 = e42.readString();
		n22[r3] = t.read(e42);
	}
	return {
		type: P.Object,
		value: n22,
	};
}
function tu(e42, r22) {
	let n22 = Object.entries(r22.value);
	for (let [r3, i22] of (e42.writeUint16(n22.length), n22))
		e42.writeString(r3), t.write(e42, i22);
}
function tl(e42, r22, n22) {
	let i22 = Object.keys(e42.value).sort(),
		s22 = Object.keys(r22.value).sort();
	if (i22.length < s22.length) return -1;
	if (i22.length > s22.length) return 1;
	for (let a22 = 0; a22 < i22.length; a22++) {
		let o22 = i22[a22],
			u22 = s22[a22];
		if (o22 < u22) return -1;
		if (o22 > u22) return 1;
		let l22 = e42.value[o22] ?? null,
			h22 = r22.value[u22] ?? null,
			c22 = t.compare(l22, h22, n22);
		if (0 !== c22) return c22;
	}
	return 0;
}
function th(t32) {
	return {
		type: P.ResponsiveImage,
		value: t32.readJson(),
	};
}
function tc(t32, e42) {
	t32.writeJson(e42.value);
}
function tf(t32, e42) {
	let r22 = JSON.stringify(t32.value),
		n22 = JSON.stringify(e42.value);
	return r22 < n22 ? -1 : r22 > n22 ? 1 : 0;
}
function td(t32) {
	let e42 = t32.readInt8();
	if (0 === e42)
		return {
			type: P.RichText,
			value: t32.readUint32(),
		};
	if (1 === e42)
		return {
			type: P.RichText,
			value: t32.readString(),
		};
	throw Error("Invalid rich text pointer");
}
function tg(t32, e42) {
	if (N(e42.value)) {
		t32.writeInt8(0), t32.writeUint32(e42.value);
		return;
	}
	if (x(e42.value)) {
		t32.writeInt8(1), t32.writeString(e42.value);
		return;
	}
	throw Error("Invalid rich text pointer");
}
function tp(t32, e42) {
	let r22 = t32.value,
		n22 = e42.value;
	if ((N(r22) && N(n22)) || (x(r22) && x(n22)))
		return r22 < n22 ? -1 : r22 > n22 ? 1 : 0;
	throw Error("Invalid rich text pointer");
}
function tv(t32) {
	return {
		type: P.String,
		value: t32.readString(),
	};
}
function ty(t32, e42) {
	t32.writeString(e42.value);
}
function tm(t32, e42, r22) {
	let n22 = t32.value,
		i22 = e42.value;
	return (0 === r22.type &&
		((n22 = t32.value.toLowerCase()), (i22 = e42.value.toLowerCase())),
	n22 < i22)
		? -1
		: n22 > i22
			? 1
			: 0;
}
function tw(t32) {
	return {
		type: P.VectorSetItem,
		value: t32.readUint32(),
	};
}
function tI(t32, e42) {
	t32.writeUint32(e42.value);
}
function tb(t32, e42) {
	let r22 = t32.value,
		n22 = e42.value;
	return r22 < n22 ? -1 : r22 > n22 ? 1 : 0;
}
((t32) => {
	(t32.read = function (t42) {
		let e42 = t42.readUint8();
		switch (e42) {
			case 0:
				return null;
			case 1:
				return q(t42);
			case 2:
				return j(t42);
			case 3:
				return V(t42);
			case 4:
				return z(t42);
			case 5:
				return H(t42);
			case 6:
				return Y(t42);
			case 7:
				return te(t42);
			case 8:
				return ti(t42);
			case 9:
				return to(t42);
			case 10:
				return th(t42);
			case 11:
				return td(t42);
			case 12:
				return tv(t42);
			case 13:
				return tw(t42);
			default:
				b(e42);
		}
	}),
		(t32.write = function (t42, e42) {
			let r22 = R(e42);
			if ((t42.writeUint8(r22), !A(e42)))
				switch (e42.type) {
					case P.Array:
						return _(t42, e42);
					case P.Boolean:
						return C(t42, e42);
					case P.Color:
						return W(t42, e42);
					case P.Date:
						return G(t42, e42);
					case P.Enum:
						return X(t42, e42);
					case P.File:
						return Z(t42, e42);
					case P.Link:
						return tr(t42, e42);
					case P.Number:
						return ts(t42, e42);
					case P.Object:
						return tu(t42, e42);
					case P.ResponsiveImage:
						return tc(t42, e42);
					case P.RichText:
						return tg(t42, e42);
					case P.VectorSetItem:
						return tI(t42, e42);
					case P.String:
						return ty(t42, e42);
					default:
						b(e42);
				}
		}),
		(t32.compare = function (t42, e42, r22) {
			let n22 = R(t42),
				i22 = R(e42);
			if (n22 < i22) return -1;
			if (n22 > i22) return 1;
			if (A(t42) || A(e42)) return 0;
			switch (t42.type) {
				case P.Array:
					return I(e42.type === P.Array), D(t42, e42, r22);
				case P.Boolean:
					return I(e42.type === P.Boolean), J(t42, e42);
				case P.Color:
					return I(e42.type === P.Color), $(t42, e42);
				case P.Date:
					return I(e42.type === P.Date), K(t42, e42);
				case P.Enum:
					return I(e42.type === P.Enum), Q(t42, e42);
				case P.File:
					return I(e42.type === P.File), tt(t42, e42);
				case P.Link:
					return I(e42.type === P.Link), tn(t42, e42);
				case P.Number:
					return I(e42.type === P.Number), ta(t42, e42);
				case P.Object:
					return I(e42.type === P.Object), tl(t42, e42, r22);
				case P.ResponsiveImage:
					return I(e42.type === P.ResponsiveImage), tf(t42, e42);
				case P.RichText:
					return I(e42.type === P.RichText), tp(t42, e42);
				case P.VectorSetItem:
					return I(e42.type === P.VectorSetItem), tb(t42, e42);
				case P.String:
					return I(e42.type === P.String), tm(t42, e42, r22);
				default:
					b(t42);
			}
		});
})(t || (t = {}));
var tU = class e2 {
	sortEntries() {
		this.entries.sort((e42, r22) => {
			for (let n22 = 0; n22 < this.fieldNames.length; n22++) {
				let i22 = e42.values[n22],
					s22 = r22.values[n22],
					a22 = t.compare(i22, s22, this.options.collation);
				if (0 !== a22) return a22;
			}
			return e42.pointer.compare(r22.pointer);
		});
	}
	static deserialize(r22) {
		let n22 = new p(r22),
			i22 = n22.readJson(),
			s22 = n22.readUint8(),
			a22 = [];
		for (let t32 = 0; t32 < s22; t32++) {
			let t42 = n22.readString();
			a22.push(t42);
		}
		let o22 = new e2(a22, {
				collation: i22,
			}),
			u22 = n22.readUint32();
		for (let e42 = 0; e42 < u22; e42++) {
			let e52 = [];
			for (let r4 = 0; r4 < s22; r4++) {
				let r5 = t.read(n22);
				e52.push(r5);
			}
			let r3 = O.read(n22);
			o22.entries.push({
				values: e52,
				pointer: r3,
			});
		}
		return o22;
	}
	serialize() {
		let e42 = new F();
		for (let t32 of (e42.writeJson(this.options.collation),
		e42.writeUint8(this.fieldNames.length),
		this.fieldNames))
			e42.writeString(t32);
		for (let r22 of (this.sortEntries(),
		e42.writeUint32(this.entries.length),
		this.entries)) {
			let { values: n22, pointer: i22 } = r22;
			for (let r3 of n22) t.write(e42, r3);
			i22.write(e42);
		}
		return e42.subarray();
	}
	addItem(t32, e42) {
		let r22 = this.fieldNames.map((e52) => t32.getField(e52) ?? null);
		this.entries.push({
			values: r22,
			pointer: e42,
		});
	}
	constructor(t32, e42) {
		(this.fieldNames = t32), (this.options = e42), c(this, "entries", []);
	}
};
var tS = 3;
var tk = 250;
var tL = [
	408,
	// Request Timeout
	429,
	// Too Many Requests
	500,
	// Internal Server Error
	502,
	// Bad Gateway
	503,
	// Service Unavailable
	504,
];
var tB = async (t32, e42) => {
	let r22 = 0;
	for (;;) {
		try {
			let n22 = await fetch(t32, e42);
			if (!tL.includes(n22.status) || ++r22 > tS) return n22;
		} catch (t42) {
			if (e42?.signal?.aborted || ++r22 > tS) throw t42;
		}
		await tE(r22);
	}
};
async function tE(t32) {
	let e42 = Math.floor(tk * (Math.random() + 1) * 2 ** (t32 - 1));
	await new Promise((t42) => {
		setTimeout(t42, e42);
	});
}
async function tM(t32, e42) {
	let r22 = tx(e42),
		n22 = [],
		i22 = 0;
	for (let t42 of r22)
		n22.push(`${t42.from}-${t42.to - 1}`), (i22 += t42.to - t42.from);
	let s22 = new URL(t32),
		a22 = n22.join(",");
	s22.searchParams.set("range", a22);
	let o22 = await tB(s22);
	if (200 !== o22.status)
		throw Error(`Request failed: ${o22.status} ${o22.statusText}`);
	let u22 = await o22.arrayBuffer(),
		l22 = new Uint8Array(u22);
	if (l22.length !== i22)
		throw Error("Request failed: Unexpected response length");
	let h22 = new tT(),
		c22 = 0;
	for (let t42 of r22) {
		let e52 = t42.to - t42.from,
			r3 = c22 + e52,
			n32 = l22.subarray(c22, r3);
		h22.write(t42.from, n32), (c22 = r3);
	}
	return e42.map((t42) => h22.read(t42.from, t42.to - t42.from));
}
var tT = class {
	read(t32, e42) {
		for (let r22 of this.chunks) {
			if (t32 < r22.start) break;
			if (t32 > r22.end) continue;
			if (t32 + e42 > r22.end) break;
			let n22 = t32 - r22.start,
				i22 = n22 + e42;
			return r22.data.slice(n22, i22);
		}
		throw Error("Missing data");
	}
	write(t32, e42) {
		let r22 = t32,
			n22 = r22 + e42.length,
			i22 = 0,
			s22 = this.chunks.length;
		for (; i22 < s22; i22++) {
			let t42 = this.chunks[i22];
			if ((I(t42, "Missing chunk"), !(r22 > t42.end))) {
				if (r22 > t42.start) {
					let n32 = r22 - t42.start,
						i32 = t42.data.subarray(0, n32);
					(e42 = tF(i32, e42)), (r22 = t42.start);
				}
				break;
			}
		}
		for (; s22 > i22; s22--) {
			let t42 = this.chunks[s22 - 1];
			if ((I(t42, "Missing chunk"), !(n22 < t42.start))) {
				if (n22 < t42.end) {
					let r3 = n22 - t42.start,
						i32 = t42.data.subarray(r3);
					(e42 = tF(e42, i32)), (n22 = t42.end);
				}
				break;
			}
		}
		let a22 = {
				start: r22,
				end: n22,
				data: e42,
			},
			o22 = s22 - i22;
		this.chunks.splice(i22, o22, a22);
	}
	constructor() {
		c(this, "chunks", []);
	}
};
function tF(t32, e42) {
	let r22 = t32.length + e42.length,
		n22 = new Uint8Array(r22);
	return n22.set(t32, 0), n22.set(e42, t32.length), n22;
}
function tx(t32) {
	I(t32.length > 0, "Must have at least one range");
	let e42 = [...t32].sort((t42, e52) => t42.from - e52.from),
		r22 = [];
	for (let t42 of e42) {
		let e52 = r22.length - 1,
			n22 = r22[e52];
		n22 && t42.from <= n22.to
			? (r22[e52] = {
					from: n22.from,
					to: Math.max(n22.to, t42.to),
				})
			: r22.push(t42);
	}
	return r22;
}
var tN = class {
	async loadModel() {
		let [t32] = await tM(this.options.url, [this.options.range]);
		return I(t32, "Failed to load model"), tU.deserialize(t32);
	}
	async getModel() {
		return (
			(this.modelPromise ??= this.loadModel()),
			(this.model ??= await this.modelPromise),
			this.model
		);
	}
	async lookupItems(t32) {
		I(t32.length === this.fields.length, "Invalid query length");
		let e42 = await this.getModel(),
			r22 = t32.reduce(
				(t42, e52, r3) =>
					t42.flatMap((t5) => {
						switch (e52.type) {
							case "All":
								return [t5];
							case "Equals":
								return this.queryEquals(t5, e52, r3);
							case "NotEquals":
								return this.queryNotEquals(t5, e52, r3);
							case "LessThan":
								return this.queryLessThan(t5, e52, r3);
							case "GreaterThan":
								return this.queryGreaterThan(t5, e52, r3);
							case "Contains":
								return this.queryContains(t5, e52, r3);
							case "StartsWith":
								return this.queryStartsWith(t5, e52, r3);
							case "EndsWith":
								return this.queryEndsWith(t5, e52, r3);
							default:
								b(e52);
						}
					}),
				[e42.entries],
			),
			n22 = [];
		for (let t42 of r22)
			for (let e52 of t42) {
				let t5 = {};
				for (let r3 = 0; r3 < this.options.fieldNames.length; r3++) {
					let n32 = this.options.fieldNames[r3],
						i22 = e52.values[r3];
					t5[n32] = i22;
				}
				n22.push({
					pointer: e52.pointer.toString(),
					data: t5,
				});
			}
		return n22;
	}
	queryEquals(t32, e42, r22) {
		let n22 = this.getLeftMost(t32, r22, e42.value),
			i22 = this.getRightMost(t32, r22, e42.value),
			s22 = t32.slice(n22, i22 + 1);
		return s22.length > 0 ? [s22] : [];
	}
	queryNotEquals(t32, e42, r22) {
		let n22 = this.getLeftMost(t32, r22, e42.value),
			i22 = this.getRightMost(t32, r22, e42.value),
			s22 = [],
			a22 = t32.slice(0, n22);
		a22.length > 0 && s22.push(a22);
		let o22 = t32.slice(i22 + 1);
		return o22.length > 0 && s22.push(o22), s22;
	}
	queryLessThan(t32, e42, r22) {
		let n22 = this.getRightMost(t32, r22, null);
		if (((t32 = t32.slice(n22 + 1)), e42.inclusive)) {
			let n32 = this.getRightMost(t32, r22, e42.value),
				i32 = t32.slice(0, n32 + 1);
			return i32.length > 0 ? [i32] : [];
		}
		let i22 = this.getLeftMost(t32, r22, e42.value),
			s22 = t32.slice(0, i22);
		return s22.length > 0 ? [s22] : [];
	}
	queryGreaterThan(t32, e42, r22) {
		let n22 = this.getRightMost(t32, r22, null);
		if (((t32 = t32.slice(n22 + 1)), e42.inclusive)) {
			let n32 = this.getLeftMost(t32, r22, e42.value),
				i32 = t32.slice(n32);
			return i32.length > 0 ? [i32] : [];
		}
		let i22 = this.getRightMost(t32, r22, e42.value),
			s22 = t32.slice(i22 + 1);
		return s22.length > 0 ? [s22] : [];
	}
	queryContains(t32, e42, r22) {
		return this.findItems(t32, r22, (t42) => {
			if (t42?.type !== y.String || e42.value?.type !== y.String) return false;
			let r3 = t42.value,
				n22 = e42.value.value;
			return (
				0 === this.collation.type &&
					((r3 = r3.toLowerCase()), (n22 = n22.toLowerCase())),
				r3.includes(n22)
			);
		});
	}
	queryStartsWith(t32, e42, r22) {
		return this.findItems(t32, r22, (t42) => {
			if (t42?.type !== y.String || e42.value?.type !== y.String) return false;
			let r3 = t42.value,
				n22 = e42.value.value;
			return (
				0 === this.collation.type &&
					((r3 = r3.toLowerCase()), (n22 = n22.toLowerCase())),
				r3.startsWith(n22)
			);
		});
	}
	queryEndsWith(t32, e42, r22) {
		return this.findItems(t32, r22, (t42) => {
			if (t42?.type !== y.String || e42.value?.type !== y.String) return false;
			let r3 = t42.value,
				n22 = e42.value.value;
			return (
				0 === this.collation.type &&
					((r3 = r3.toLowerCase()), (n22 = n22.toLowerCase())),
				r3.endsWith(n22)
			);
		});
	}
	/**
	 * Returns the index of the left most entry that is equal to the target.
	 *
	 * ```text
	 *   Left most
	 *       ↓
	 * ┌───┬───┬───┬───┬───┬───┐
	 * │ 1 │ 2 │ 2 │ 2 │ 2 │ 3 │
	 * └───┴───┴───┴───┴───┴───┘
	 * ```
	 *
	 * @param entries The entries array to search in.
	 * @param position The position of the value in the entry.
	 * @param target The target value to search for.
	 * @returns The index of the left most entry that is equal to the target.
	 */
	getLeftMost(e42, r22, n22) {
		let i22 = 0,
			s22 = e42.length;
		for (; i22 < s22; ) {
			let a22 = (i22 + s22) >> 1,
				o22 = e42[a22],
				u22 = o22.values[r22];
			0 > t.compare(u22, n22, this.collation) ? (i22 = a22 + 1) : (s22 = a22);
		}
		return i22;
	}
	/**
	 * Returns the index of the right most entry that is equal to the target.
	 *
	 * ```text
	 *              Right most
	 *                   ↓
	 * ┌───┬───┬───┬───┬───┬───┐
	 * │ 1 │ 2 │ 2 │ 2 │ 2 │ 3 │
	 * └───┴───┴───┴───┴───┴───┘
	 * ```
	 *
	 * @param entries The entries array to search in.
	 * @param position The position of the value in the entry.
	 * @param target The target value to search for.
	 * @returns The index of the right most entry that is equal to the target.
	 */
	getRightMost(e42, r22, n22) {
		let i22 = 0,
			s22 = e42.length;
		for (; i22 < s22; ) {
			let a22 = (i22 + s22) >> 1,
				o22 = e42[a22],
				u22 = o22.values[r22];
			t.compare(u22, n22, this.collation) > 0 ? (s22 = a22) : (i22 = a22 + 1);
		}
		return s22 - 1;
	}
	/**
	 * Finds all items that are matching the predicate and groups adjacent items together.
	 *
	 * @param entries The entries array to search in.
	 * @param position The position of the value in the entry.
	 * @param predicate The predicate to match the values against.
	 * @returns An array of chunks that match the predicate.
	 */
	findItems(t32, e42, r22) {
		let n22 = [],
			i22 = 0;
		for (let s22 = 0; s22 < t32.length; s22++) {
			let a22 = t32[s22],
				o22 = a22.values[e42],
				u22 = r22(o22);
			if (!u22) {
				if (i22 < s22) {
					let e52 = t32.slice(i22, s22);
					n22.push(e52);
				}
				i22 = s22 + 1;
			}
		}
		if (i22 < t32.length) {
			let e52 = t32.slice(i22);
			n22.push(e52);
		}
		return n22;
	}
	constructor(t32) {
		(this.options = t32),
			c(this, "schema"),
			c(this, "fields"),
			c(this, "supportedLookupTypes", [
				"All",
				"Equals",
				"NotEquals",
				"LessThan",
				"GreaterThan",
				"Contains",
				"StartsWith",
				"EndsWith",
				/* EndsWith */
			]),
			c(this, "modelPromise"),
			c(this, "model"),
			c(this, "collation");
		let e42 = {},
			r22 = [];
		for (let t42 of this.options.fieldNames) {
			let n22 = this.options.collectionSchema[t42];
			I(n22, "Missing definition for field", t42),
				(e42[t42] = n22),
				r22.push({
					type: "Identifier",
					name: t42,
				});
		}
		(this.schema = e42),
			(this.fields = r22),
			(this.collation = this.options.collation);
	}
};
var tA = class e3 {
	static read(r22) {
		let n22 = new e3(),
			i22 = r22.readUint16();
		for (let e42 = 0; e42 < i22; e42++) {
			let e52 = r22.readString(),
				i32 = t.read(r22);
			n22.setField(e52, i32);
		}
		return n22;
	}
	write(e42) {
		for (let [r22, n22] of (e42.writeUint16(this.fields.size), this.fields))
			e42.writeString(r22), t.write(e42, n22);
	}
	getData() {
		let t32 = {};
		for (let [e42, r22] of this.fields) t32[e42] = r22;
		return t32;
	}
	setField(t32, e42) {
		this.fields.set(t32, e42);
	}
	getField(t32) {
		return this.fields.get(t32);
	}
	constructor() {
		c(this, "fields", /* @__PURE__ */ new Map());
	}
};
var tO = class {
	scanItems() {
		return (
			(this.itemsPromise ??= tB(this.url).then(async (t32) => {
				if (!t32.ok)
					throw Error(`Request failed: ${t32.status} ${t32.statusText}`);
				let e42 = await t32.arrayBuffer(),
					r22 = new Uint8Array(e42),
					n22 = new p(r22),
					i22 = [],
					s22 = n22.readUint32();
				for (let t42 = 0; t42 < s22; t42++) {
					let t5 = n22.getOffset(),
						e52 = tA.read(n22),
						r3 = n22.getOffset() - t5,
						s32 = new O(this.id, t5, r3),
						a22 = s32.toString(),
						o22 = {
							pointer: a22,
							data: e52.getData(),
						};
					this.itemLoader.prime(a22, o22), i22.push(o22);
				}
				return i22;
			})),
			this.itemsPromise
		);
	}
	resolveItem(t32) {
		return this.itemLoader.load(t32);
	}
	constructor(t32, e42) {
		(this.id = t32),
			(this.url = e42),
			c(this, "itemsPromise"),
			c(
				this,
				"itemLoader",
				new d.default(
					async (t42) => {
						let e52 = t42.map((t5) => {
								let e6 = O.fromString(t5);
								return {
									from: e6.offset,
									to: e6.offset + e6.length,
								};
							}),
							r22 = await tM(this.url, e52);
						return r22.map((e6, r3) => {
							let n22 = new p(e6),
								i22 = tA.read(n22),
								s22 = t42[r3];
							return (
								I(s22, "Missing pointer"),
								{
									pointer: s22,
									data: i22.getData(),
								}
							);
						});
					},
					{
						maxBatchSize: 500,
					},
				),
			);
	}
};
var tP = class {
	async scanItems() {
		let t32 = await Promise.all(
			this.chunks.map(async (t42) => t42.scanItems()),
		);
		return t32.flat();
	}
	resolveItems(t32) {
		return Promise.all(
			t32.map((t42) => {
				let e42 = O.fromString(t42),
					r22 = this.chunks[e42.chunkId];
				return I(r22, "Missing chunk"), r22.resolveItem(t42);
			}),
		);
	}
	compareItems(t32, e42) {
		let r22 = O.fromString(t32.pointer),
			n22 = O.fromString(e42.pointer);
		return r22.compare(n22);
	}
	compareValues(e42, r22, n22) {
		return t.compare(e42, r22, n22);
	}
	constructor(t32) {
		(this.options = t32),
			c(this, "id"),
			c(this, "schema"),
			c(this, "indexes"),
			c(this, "resolveRichText"),
			c(this, "resolveVectorSetItem"),
			c(this, "chunks"),
			(this.chunks = this.options.chunks.map((t42, e42) => new tO(e42, t42))),
			(this.schema = t32.schema),
			(this.indexes = t32.indexes),
			(this.resolveRichText = t32.resolveRichText),
			(this.resolveVectorSetItem = t32.resolveVectorSetItem),
			(this.id = t32.id);
	}
};

// /:https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch-1.js
import { jsx as e4 } from "react/jsx-runtime";
import {
	AutoBreakpointVariant as t3,
	ComponentPresetsConsumer as r2,
	Link as n2,
	motion as o2,
} from "unframer";
import { isValidElement as i2 } from "react";
import { Fragment as p2, createElement as s2 } from "react";
var __unframerWindow2 = typeof window !== "undefined" ? window : void 0;
var a2;
var l2 = "undefined" != typeof __unframerWindow2;
var u2 = l2 && "function" == typeof __unframerWindow2.requestIdleCallback;
var c2 = "preload";
function f2(e22) {
	return (
		"object" == typeof e22 &&
		null !== e22 &&
		!(/* @__PURE__ */ i2(e22)) &&
		c2 in e22
	);
}
function m2(e22, ...t22) {
	if (!e22)
		throw Error(
			"Assertion Error" + (t22.length > 0 ? ": " + t22.join(" ") : ""),
		);
}
var d2 =
	(((a2 = d2 || {})[(a2.Fragment = 1)] = "Fragment"),
	(a2[(a2.Link = 2)] = "Link"),
	(a2[(a2.Module = 3)] = "Module"),
	(a2[(a2.Tag = 4)] = "Tag"),
	(a2[(a2.Text = 5)] = "Text"),
	a2);
function g2(i22) {
	let a22 = /* @__PURE__ */ new Map();
	return (l22) => {
		let u22 = a22.get(l22);
		if (u22) return u22;
		let c22 = JSON.parse(l22),
			d22 = (function a3(l32) {
				switch (l32[0]) {
					case 1: {
						let [, ...e22] = l32,
							t22 = e22.map(a3);
						return s2(p2, void 0, ...t22);
					}
					case 2: {
						let [, e22, ...t22] = l32,
							r22 = t22.map(a3);
						return s2(n2, e22, ...r22);
					}
					case 3: {
						let [, n22, o22, u32] = l32;
						for (let e22 of u32) {
							let t22 = o22[e22];
							t22 && (o22[e22] = a3(t22));
						}
						let c32 = i22[n22];
						return (
							m2(c32, "Module not found"),
							f2(c32) && c32.preload(),
							e4(r2, {
								componentIdentifier: n22,
								children: (r22) =>
									e4(t3, {
										component: c32,
										props: {
											...r22,
											...o22,
										},
									}),
							})
						);
					}
					case 4: {
						let [, e22, t22, ...r22] = l32,
							n22 = r22.map(a3);
						if ("a" === e22) return <o2.a />;
						return s2(e22, t22, ...n22);
					}
					case 5: {
						let [, e22] = l32;
						return e22;
					}
				}
			})(c22);
		return a22.set(l22, d22), d22;
	};
}

// /:https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js
var i3 = {
	cbdsG1kwf: {
		isNullable: true,
		type: l3.String,
	},
	cHUmcyRnt: {
		isNullable: true,
		type: l3.Enum,
	},
	createdAt: {
		isNullable: true,
		type: l3.Date,
	},
	EqhQyfIbD: {
		isNullable: true,
		type: l3.Link,
	},
	id: {
		isNullable: false,
		type: l3.String,
	},
	IvnecNHdQ: {
		isNullable: true,
		type: l3.String,
	},
	mqVPWrVZQ: {
		isNullable: true,
		type: l3.String,
	},
	NaKxbk01L: {
		isNullable: true,
		type: l3.Date,
	},
	nbsfKpHfS: {
		isNullable: true,
		type: l3.RichText,
	},
	nextItemId: {
		isNullable: true,
		type: l3.String,
	},
	phek70ei_: {
		isNullable: true,
		type: l3.RichText,
	},
	previousItemId: {
		isNullable: true,
		type: l3.String,
	},
	T1AgJrcWB: {
		isNullable: true,
		type: l3.String,
	},
	u3Z9wFNjT: {
		isNullable: true,
		type: l3.String,
	},
	updatedAt: {
		isNullable: true,
		type: l3.Date,
	},
	uS7iNeSm_: {
		isNullable: true,
		type: l3.ResponsiveImage,
	},
	w7LSuE_G0: {
		definition: {
			isNullable: true,
			type: l3.String,
		},
		isNullable: true,
		type: l3.Array,
	},
	xn7vebUYt: {
		isNullable: true,
		type: l3.Boolean,
	},
	z9OOU4Sta: {
		isNullable: true,
		type: l3.String,
	},
};
var o3 = ["id"];
var c3 = {
	type: 1,
};
var n3 = ["previousItemId"];
var u3 = ["nextItemId"];
var s3 = ["id", "IvnecNHdQ"];
var d3 = ["IvnecNHdQ", "id"];
var f3 = ["T1AgJrcWB"];
var p3 = {
	type: 0,
};
var y2 = ["IvnecNHdQ"];
var h2 = ["NaKxbk01L"];
var N2 = ["xn7vebUYt"];
var g3 = ["cHUmcyRnt"];
var w2 = ["w7LSuE_G0"];
var S2 = ["uS7iNeSm_"];
var x2 = ["mqVPWrVZQ"];
var L2 = ["cbdsG1kwf"];
var D2 = ["u3Z9wFNjT"];
var b2 = ["nbsfKpHfS"];
var I2 = ["phek70ei_"];
var R2 = ["z9OOU4Sta"];
var U2 = ["EqhQyfIbD"];
var v2 = [];
var k2 = (e22) => {
	let l22 = v2[e22];
	if (l22) return l22().then((e32) => e32.default);
};
var V2 = {};
var H2 = g2(V2);
var Q2 = new t4();
var T2 = {
	collectionByLocaleId: {
		default: new tP({
			chunks: [
				new URL(
					"./l2oD2amch-chunk-default-0.framercms",
					"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
				).href.replace("/modules/", "/cms/"),
			],
			id: "55633796-1bf4-4348-90e8-b99da8a48f03default",
			indexes: [
				new tN({
					collation: c3,
					collectionSchema: i3,
					fieldNames: o3,
					range: {
						from: 0,
						to: 1177,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: c3,
					collectionSchema: i3,
					fieldNames: n3,
					range: {
						from: 1177,
						to: 2353,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: c3,
					collectionSchema: i3,
					fieldNames: u3,
					range: {
						from: 2353,
						to: 3525,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: c3,
					collectionSchema: i3,
					fieldNames: s3,
					range: {
						from: 3525,
						to: 5791,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: c3,
					collectionSchema: i3,
					fieldNames: d3,
					range: {
						from: 5791,
						to: 8057,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: f3,
					range: {
						from: 8057,
						to: 9645,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: y2,
					range: {
						from: 9645,
						to: 11233,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: h2,
					range: {
						from: 11233,
						to: 12177,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: N2,
					range: {
						from: 12177,
						to: 12785,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: g3,
					range: {
						from: 12785,
						to: 13969,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: w2,
					range: {
						from: 13969,
						to: 15969,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: S2,
					range: {
						from: 15969,
						to: 40797,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: x2,
					range: {
						from: 40797,
						to: 41808,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: L2,
					range: {
						from: 41808,
						to: 42605,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: D2,
					range: {
						from: 42605,
						to: 43429,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: b2,
					range: {
						from: 43429,
						to: 48639,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: I2,
					range: {
						from: 48639,
						to: 76836,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: R2,
					range: {
						from: 76836,
						to: 78171,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
				new tN({
					collation: p3,
					collectionSchema: i3,
					fieldNames: U2,
					range: {
						from: 78171,
						to: 83339,
					},
					url: new URL(
						"./l2oD2amch-indexes-default-0.framercms",
						"https://framerusercontent.com/modules/NZPFSw2NCOL68hT9rpwN/MBnSfDlr5KfiP2p61ivs/l2oD2amch.js",
					).href.replace("/modules/", "/cms/"),
				}),
			],
			resolveRichText: H2,
			resolveVectorSetItem: k2,
			schema: i3,
		}),
	},
	displayName: "Products",
	id: "55633796-1bf4-4348-90e8-b99da8a48f03",
};
var stdin_default2 = T2;
e5(T2, {
	T1AgJrcWB: {
		defaultValue: "",
		title: "Title",
		type: l3.String,
	},
	IvnecNHdQ: {
		preventLocalization: false,
		title: "Slug",
		type: l3.String,
	},
	NaKxbk01L: {
		defaultValue: "",
		title: "Date",
		type: l3.Date,
	},
	xn7vebUYt: {
		defaultValue: false,
		title: "Popular",
		type: l3.Boolean,
	},
	cHUmcyRnt: {
		defaultValue: "yLGTkmjWx",
		options: ["yLGTkmjWx", "vH6RYiJk0", "YrAyB9mQx", "JeYGCItsL", "CQkLsagex"],
		optionTitles: ["Pizza", "Pasta", "Sides", "Deserts", "Drinks"],
		title: "Category",
		type: l3.Enum,
	},
	w7LSuE_G0: {
		dataIdentifier: "local-module:collection/M5XiN7F5c:default",
		title: "Menus",
		type: l3.MultiCollectionReference,
	},
	uS7iNeSm_: {
		title: "Image",
		type: l3.ResponsiveImage,
	},
	mqVPWrVZQ: {
		defaultValue: "",
		title: "Price - Medium",
		type: l3.String,
	},
	cbdsG1kwf: {
		defaultValue: "",
		title: "Portion",
		type: l3.String,
	},
	u3Z9wFNjT: {
		defaultValue: "",
		title: "Price - Large",
		type: l3.String,
	},
	nbsfKpHfS: {
		defaultValue: "",
		title: "Ingredients",
		type: l3.RichText,
	},
	phek70ei_: {
		defaultValue: "",
		title: "Nutritional Information",
		type: l3.RichText,
	},
	z9OOU4Sta: {
		defaultValue: "",
		title: "Alergen Warnings",
		type: l3.String,
	},
	EqhQyfIbD: {
		title: "Delivery Link",
		type: l3.Link,
	},
	createdAt: {
		title: "Created",
		type: l3.Date,
	},
	updatedAt: {
		title: "Updated",
		type: l3.Date,
	},
	previousItemId: {
		dataIdentifier: "local-module:collection/l2oD2amch:default",
		title: "Previous",
		type: l3.CollectionReference,
	},
	nextItemId: {
		dataIdentifier: "local-module:collection/l2oD2amch:default",
		title: "Next",
		type: l3.CollectionReference,
	},
});

// /:https://framerusercontent.com/modules/DfYppUtUo820MBg84hlC/0NR3eUS4cESI5fGc6hJI/jBpmYi8dY.js
import { fontStore } from "unframer";
fontStore.loadFonts([
	"Inter",
	"Inter-Bold",
	"Inter-BoldItalic",
	"Inter-Italic",
]);
var fonts4 = [
	{
		explicitInter: true,
		fonts: [
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/GrgcKwrN6d3Uz8EwcLHZxwEfC4.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/DpPBYI0sL4fYLgAkX8KXOPVt7c.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/4RAEQdEOrcnDkhHiiCbJOw92Lk.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/1K3W8DizY3v4emK8Mb08YHxTbs.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/tUSCtfYVM1I1IchuyCwz9gDdQ.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/VgYFWiwsAC5OYxAycRXXvhze58.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/syRNPWzAMIrcJ3wIlPIP43KjQs.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "normal",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/GIryZETIX4IFypco5pYZONKhJIo.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/H89BbHkbHDzlxZzxi8uPzTsp90.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/u6gJwDuwB143kpNK1T1MDKDWkMc.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/43sJ6MfOPh1LCJt46OvyDuSbA6o.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/wccHG0r4gBDAIRhfHiOlq6oEkqw.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/WZ367JPwf9bRW6LdTHN8rXgSjw.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/ia3uin3hQWqDrVloC1zEtYHWw.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/2A4Xx7CngadFGlVV4xrO06OBHY.woff2",
				weight: "700",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/CfMzU8w2e7tHgF4T4rATMPuWosA.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/867QObYax8ANsfX4TGEVU9YiCM.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/Oyn2ZbENFdnW7mt2Lzjk1h9Zb9k.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/cdAe8hgZ1cMyLu9g005pAW3xMo.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/DOfvtmE1UplCq161m6Hj8CSQYg.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/pKRFNWFoZl77qYCAIp84lN1h944.woff2",
				weight: "400",
			},
			{
				cssFamilyName: "Inter",
				source: "framer",
				style: "italic",
				uiFamilyName: "Inter",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/tKtBcDnBMevsEEJKdNGhhkLzYo.woff2",
				weight: "400",
			},
		],
	},
];
var css4 = [
	'.framer-cFKF8 .framer-styles-preset-1bh5kbf:not(.rich-text-wrapper), .framer-cFKF8 .framer-styles-preset-1bh5kbf.rich-text-wrapper p { --framer-font-family: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 20px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: italic; --framer-font-style-italic: italic; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-font-weight-bold: 700; --framer-font-weight-bold-italic: 700; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.04em; --framer-line-height: 1.4em; --framer-paragraph-spacing: 12px; --framer-text-alignment: center; --framer-text-color: var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, #333333); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
	'@media (max-width: 1199px) and (min-width: 810px) { .framer-cFKF8 .framer-styles-preset-1bh5kbf:not(.rich-text-wrapper), .framer-cFKF8 .framer-styles-preset-1bh5kbf.rich-text-wrapper p { --framer-font-family: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 18px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: italic; --framer-font-style-italic: italic; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-font-weight-bold: 700; --framer-font-weight-bold-italic: 700; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.04em; --framer-line-height: 1.4em; --framer-paragraph-spacing: 12px; --framer-text-alignment: center; --framer-text-color: var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, #333333); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
	'@media (max-width: 809px) and (min-width: 0px) { .framer-cFKF8 .framer-styles-preset-1bh5kbf:not(.rich-text-wrapper), .framer-cFKF8 .framer-styles-preset-1bh5kbf.rich-text-wrapper p { --framer-font-family: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 16px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: italic; --framer-font-style-italic: italic; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-font-weight-bold: 700; --framer-font-weight-bold-italic: 700; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.04em; --framer-line-height: 1.4em; --framer-paragraph-spacing: 12px; --framer-text-alignment: center; --framer-text-color: var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, #333333); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
];
var className4 = "framer-cFKF8";

// /:https://framerusercontent.com/modules/pnF1PIP9Z7Xyk1a22RvS/PFMJcPlHqgSyurqUU0TD/sdO1Bzv_P.js
var ImageWithFX = withFX(Image);
var ButtonFonts = getFonts(stdin_default);
var cycleOrder = [
	"JlqgQhEnq",
	"MMad5lWE1",
	"RxVeqV7K2",
	"JMpjGRmKa",
	"MUdLR3PJV",
	"E7X05txrn",
	"BgVKR3peo",
	"w3dbh6gAh",
	"OquFlA0G4",
	"MTxAdEQft",
	"kxfVpU5z6",
	"noJGfd7Q9",
];
var serializationHash = "framer-odiKA";
var variantClassNames = {
	BgVKR3peo: "framer-v-1uic2k7",
	E7X05txrn: "framer-v-106v0r8",
	JlqgQhEnq: "framer-v-xcd8td",
	JMpjGRmKa: "framer-v-1e6ewmd",
	kxfVpU5z6: "framer-v-1o1vf8b",
	MMad5lWE1: "framer-v-1gf4zxv",
	MTxAdEQft: "framer-v-7sm5nt",
	MUdLR3PJV: "framer-v-rjnsuh",
	noJGfd7Q9: "framer-v-1m3c57k",
	OquFlA0G4: "framer-v-1wr12ct",
	RxVeqV7K2: "framer-v-1cfwlvq",
	w3dbh6gAh: "framer-v-4u0o7z",
};
function addPropertyOverrides(overrides, ...variants) {
	const nextOverrides = {};
	variants?.forEach(
		(variant) => variant && Object.assign(nextOverrides, overrides[variant]),
	);
	return nextOverrides;
}
var transition1 = {
	damping: 60,
	delay: 0,
	mass: 1,
	stiffness: 500,
	type: "spring",
};
var toResponsiveImage = (value) => {
	if (
		typeof value === "object" &&
		value !== null &&
		typeof value.src === "string"
	) {
		return value;
	}
	return typeof value === "string"
		? {
				src: value,
			}
		: void 0;
};
var query1 = () => ({
	from: {
		alias: "lKLD2qPxR",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "lKLD2qPxR",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "lKLD2qPxR",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Firecracker Inferno",
		},
		type: "BinaryOperation",
	},
});
var query3 = () => ({
	from: {
		alias: "lKLD2qPxR",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "lKLD2qPxR",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "lKLD2qPxR",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Cheese Avalanche",
		},
		type: "BinaryOperation",
	},
});
var query5 = () => ({
	from: {
		alias: "lKLD2qPxR",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "lKLD2qPxR",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "lKLD2qPxR",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Mediterranean Marvel",
		},
		type: "BinaryOperation",
	},
});
var query7 = () => ({
	from: {
		alias: "lKLD2qPxR",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "lKLD2qPxR",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "lKLD2qPxR",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Hawaiian Heatwave",
		},
		type: "BinaryOperation",
	},
});
var query9 = () => ({
	from: {
		alias: "lKLD2qPxR",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "lKLD2qPxR",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "lKLD2qPxR",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "lKLD2qPxR",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Meat Lover's Feast",
		},
		type: "BinaryOperation",
	},
});
var QueryData = ({ query: query20, pageSize, children }) => {
	const data = useQueryData(query20);
	return children(data);
};
var query11 = () => ({
	from: {
		alias: "dihO8NRkz",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "dihO8NRkz",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "dihO8NRkz",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Buffalo Bliss",
		},
		type: "BinaryOperation",
	},
});
var query13 = () => ({
	from: {
		alias: "dihO8NRkz",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "dihO8NRkz",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "dihO8NRkz",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Truffle Temptation",
		},
		type: "BinaryOperation",
	},
});
var query15 = () => ({
	from: {
		alias: "dihO8NRkz",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "dihO8NRkz",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "dihO8NRkz",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Garlic Supreme",
		},
		type: "BinaryOperation",
	},
});
var query17 = () => ({
	from: {
		alias: "dihO8NRkz",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "dihO8NRkz",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "dihO8NRkz",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "Pepperoni Popper",
		},
		type: "BinaryOperation",
	},
});
var query19 = () => ({
	from: {
		alias: "dihO8NRkz",
		data: stdin_default2,
		type: "Collection",
	},
	limit: {
		type: "LiteralValue",
		value: 1,
	},
	select: [
		{
			collection: "dihO8NRkz",
			name: "uS7iNeSm_",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "IvnecNHdQ",
			type: "Identifier",
		},
		{
			collection: "dihO8NRkz",
			name: "id",
			type: "Identifier",
		},
	],
	where: {
		left: {
			collection: "dihO8NRkz",
			name: "T1AgJrcWB",
			type: "Identifier",
		},
		operator: "==",
		right: {
			type: "LiteralValue",
			value: "BBQ Blaze",
		},
		type: "BinaryOperation",
	},
});
var matchVariant = (...args) => {
	for (const arg of args) {
		if (arg && typeof arg === "string") return arg;
	}
	return void 0;
};
var Transition = ({ value, children }) => {
	const config = React.useContext(MotionConfigContext);
	const transition = value ?? config.transition;
	const contextValue = React.useMemo(
		() => ({
			...config,
			transition,
		}),
		[JSON.stringify(transition)],
	);
	return (
		<MotionConfigContext.Provider value={contextValue}>
			{children}
		</MotionConfigContext.Provider>
	);
};
var humanReadableVariantMap = {
	"Cheese Lovers Pair - Mobile": "w3dbh6gAh",
	"Cheese Lovers Pair": "MMad5lWE1",
	"Meat Feast Combo - Big": "E7X05txrn",
	"Meat Feast Combo - Mobile - Tablet": "noJGfd7Q9",
	"Meat Feast Combo - Mobile": "kxfVpU5z6",
	"Meat Feast Combo": "MUdLR3PJV",
	"Spicy Duo Deal - Mobile": "BgVKR3peo",
	"Spicy Duo Deal": "JlqgQhEnq",
	"Sweet & Savory Combo - Mobile": "MTxAdEQft",
	"Sweet & Savory Combo": "JMpjGRmKa",
	"Veggie Delight Duo - Mobile": "OquFlA0G4",
	"Veggie Delight Duo": "RxVeqV7K2",
};
var Variants = motion.create(React.Fragment);
var getProps = ({ height, id, width, ...props }) => {
	return {
		...props,
		variant:
			humanReadableVariantMap[props.variant] ?? props.variant ?? "JlqgQhEnq",
	};
};
var createLayoutDependency = (props, variants) => {
	if (props.layoutDependency)
		return variants.join("-") + props.layoutDependency;
	return variants.join("-");
};
var Component = /* @__PURE__ */ React.forwardRef(function (props, ref) {
	const fallbackRef = useRef(null);
	const refBinding = ref ?? fallbackRef;
	const defaultLayoutId = React.useId();
	const { activeLocale, setLocale } = useLocaleInfo();
	const componentViewport = useComponentViewport();
	const {
		style,
		className: className5,
		layoutId,
		variant,
		...restProps
	} = getProps(props);
	const {
		baseVariant,
		classNames,
		clearLoadingGesture,
		gestureHandlers,
		gestureVariant,
		isLoading,
		setGestureState,
		setVariant,
		variants,
	} = useVariantState({
		cycleOrder,
		defaultVariant: "JlqgQhEnq",
		ref: refBinding,
		variant,
		variantClassNames,
	});
	const layoutDependency = createLayoutDependency(props, variants);
	const sharedStyleClassNames = [className3, className4, className, className2];
	const scopingClassNames = cx(serializationHash, ...sharedStyleClassNames);
	return (
		<LayoutGroup id={layoutId ?? defaultLayoutId}>
			<Variants animate={variants} initial={false}>
				<Transition value={transition1}>
					<motion.div
						{...restProps}
						{...gestureHandlers}
						className={cx(
							scopingClassNames,
							"framer-xcd8td",
							className5,
							classNames,
						)}
						data-framer-name={"Spicy Duo Deal"}
						layoutDependency={layoutDependency}
						layoutId={"JlqgQhEnq"}
						ref={refBinding}
						style={{
							backgroundColor:
								"var(--token-1460a2d2-639e-48c8-aee5-96e4ffb1e8ff, rgb(255, 0, 60))",
							borderBottomLeftRadius: 24,
							borderBottomRightRadius: 24,
							borderTopLeftRadius: 24,
							borderTopRightRadius: 24,
							...style,
						}}
						variants={{
							E7X05txrn: {
								backgroundColor:
									"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
							},
							JMpjGRmKa: {
								backgroundColor:
									"var(--token-5ab4d244-cc9a-4962-8c8a-8ffe470a65db, rgb(255, 145, 0))",
							},
							kxfVpU5z6: {
								backgroundColor:
									"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
							},
							MMad5lWE1: {
								backgroundColor:
									"var(--token-c6c9b696-45cb-45f1-9900-e63f6c7a417f, rgb(255, 204, 0))",
							},
							MTxAdEQft: {
								backgroundColor:
									"var(--token-5ab4d244-cc9a-4962-8c8a-8ffe470a65db, rgb(255, 145, 0))",
							},
							MUdLR3PJV: {
								backgroundColor:
									"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
							},
							noJGfd7Q9: {
								backgroundColor:
									"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
							},
							OquFlA0G4: {
								backgroundColor:
									"var(--token-a5863725-53d2-46c7-a6b3-c04cec916a40, rgb(10, 153, 0))",
							},
							RxVeqV7K2: {
								backgroundColor:
									"var(--token-a5863725-53d2-46c7-a6b3-c04cec916a40, rgb(10, 153, 0))",
							},
							w3dbh6gAh: {
								backgroundColor:
									"var(--token-c6c9b696-45cb-45f1-9900-e63f6c7a417f, rgb(255, 204, 0))",
							},
						}}
						{...addPropertyOverrides(
							{
								BgVKR3peo: {
									"data-framer-name": "Spicy Duo Deal - Mobile",
								},
								E7X05txrn: {
									"data-framer-name": "Meat Feast Combo - Big",
								},
								JMpjGRmKa: {
									"data-framer-name": "Sweet & Savory Combo",
								},
								kxfVpU5z6: {
									"data-framer-name": "Meat Feast Combo - Mobile",
								},
								MMad5lWE1: {
									"data-framer-name": "Cheese Lovers Pair",
								},
								MTxAdEQft: {
									"data-framer-name": "Sweet & Savory Combo - Mobile",
								},
								MUdLR3PJV: {
									"data-framer-name": "Meat Feast Combo",
								},
								noJGfd7Q9: {
									"data-framer-name": "Meat Feast Combo - Mobile - Tablet",
								},
								OquFlA0G4: {
									"data-framer-name": "Veggie Delight Duo - Mobile",
								},
								RxVeqV7K2: {
									"data-framer-name": "Veggie Delight Duo",
								},
								w3dbh6gAh: {
									"data-framer-name": "Cheese Lovers Pair - Mobile",
								},
							},
							baseVariant,
							gestureVariant,
						)}
					>
						<RichText
							__fromCanvasComponent={true}
							className={"framer-1wrnumo"}
							fonts={["Inter"]}
							layoutDependency={layoutDependency}
							layoutId={"jQqds5Dlc"}
							style={{
								"--extracted-a0htzi":
									"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
								"--framer-link-text-color": "rgb(0, 153, 255)",
								"--framer-link-text-decoration": "underline",
							}}
							variants={{
								MMad5lWE1: {
									"--extracted-a0htzi":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
								},
								w3dbh6gAh: {
									"--extracted-a0htzi":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
								},
							}}
							verticalAlignment={"top"}
							withExternalLayout={true}
							{...addPropertyOverrides(
								{
									E7X05txrn: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Meat Feast Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									JMpjGRmKa: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Sweet & Savory Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									kxfVpU5z6: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Meat Feast Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									MMad5lWE1: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													{"Cheese Lovers Pair"}
												</motion.h3>
											</React.Fragment>
										),
									},
									MTxAdEQft: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Sweet & Savory Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									MUdLR3PJV: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Meat Feast Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									noJGfd7Q9: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Meat Feast Combo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									OquFlA0G4: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Veggie Delight Duo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									RxVeqV7K2: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													{"Veggie Delight Duo"}
												</motion.h3>
											</React.Fragment>
										),
									},
									w3dbh6gAh: {
										children: (
											<React.Fragment>
												<motion.h3
													className={"framer-styles-preset-1qlh3s0"}
													data-styles-preset={"v29eFMwtk"}
													style={{
														"--framer-text-color":
															"var(--extracted-a0htzi, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													{"Cheese Lovers Pair"}
												</motion.h3>
											</React.Fragment>
										),
									},
								},
								baseVariant,
								gestureVariant,
							)}
						>
							<React.Fragment>
								<motion.h3
									className={"framer-styles-preset-1qlh3s0"}
									data-styles-preset={"v29eFMwtk"}
									dir={"auto"}
									style={{
										"--framer-text-color":
											"var(--extracted-a0htzi, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
									}}
								>
									{"Spicy Duo Deal"}
								</motion.h3>
							</React.Fragment>
						</RichText>
						<RichText
							__fromCanvasComponent={true}
							className={"framer-1jg7xgw"}
							fonts={["Inter"]}
							layoutDependency={layoutDependency}
							layoutId={"tbZG_p3ef"}
							style={{
								"--extracted-164u4qx":
									"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
								"--extracted-1l7ec2w":
									"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
								"--extracted-1ok3k95":
									"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
								"--extracted-oa5vkk":
									"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
								"--framer-link-text-color": "rgb(0, 153, 255)",
								"--framer-link-text-decoration": "underline",
							}}
							variants={{
								MMad5lWE1: {
									"--extracted-164u4qx":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-1l7ec2w":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-1ok3k95":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-oa5vkk":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
								},
								w3dbh6gAh: {
									"--extracted-164u4qx":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-1l7ec2w":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-1ok3k95":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
									"--extracted-oa5vkk":
										"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
								},
							}}
							verticalAlignment={"top"}
							withExternalLayout={true}
							{...addPropertyOverrides(
								{
									E7X05txrn: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Meat Lover's Feast"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium BBQ Blaze"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									JMpjGRmKa: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Hawaiian Heatwave"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Pepperoni Popper"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									kxfVpU5z6: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Meat Lover's Feast"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium BBQ Blaze"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									MMad5lWE1: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"1 Medium Cheese Avalanche"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"1 Medium Truffle Temptation"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									MTxAdEQft: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Hawaiian Heatwave"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Pepperoni Popper"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									MUdLR3PJV: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Meat Lover's Feast"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium BBQ Blaze"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									noJGfd7Q9: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Meat Lover's Feast"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium BBQ Blaze"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									OquFlA0G4: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Mediterranean Marvel"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Garlic Supreme"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									RxVeqV7K2: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Mediterranean Marvel"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"1 Medium Garlic Supreme"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
									w3dbh6gAh: {
										children: (
											<React.Fragment>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1l7ec2w, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-164u4qx, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"1 Medium Cheese Avalanche"}
														</motion.p>
													</motion.li>
												</motion.ul>
												<motion.ul
													className={"framer-styles-preset-1bh5kbf"}
													data-styles-preset={"jBpmYi8dY"}
													style={{
														"--framer-text-alignment": "left",
														"--framer-text-color":
															"var(--extracted-1ok3k95, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
													}}
												>
													<motion.li>
														<motion.p
															style={{
																"--framer-text-alignment": "left",
																"--framer-text-color":
																	"var(--extracted-oa5vkk, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"1 Medium Truffle Temptation"}
														</motion.p>
													</motion.li>
												</motion.ul>
											</React.Fragment>
										),
									},
								},
								baseVariant,
								gestureVariant,
							)}
						>
							<React.Fragment>
								<motion.ul
									className={"framer-styles-preset-1bh5kbf"}
									data-styles-preset={"jBpmYi8dY"}
									dir={"auto"}
									style={{
										"--framer-text-alignment": "left",
										"--framer-text-color":
											"var(--extracted-1l7ec2w, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
									}}
								>
									<motion.li data-preset-tag={"p"}>
										<motion.p
											style={{
												"--framer-text-alignment": "left",
												"--framer-text-color":
													"var(--extracted-164u4qx, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
											}}
										>
											{"1 Medium Firecracker Inferno "}
										</motion.p>
									</motion.li>
								</motion.ul>
								<motion.ul
									className={"framer-styles-preset-1bh5kbf"}
									data-styles-preset={"jBpmYi8dY"}
									dir={"auto"}
									style={{
										"--framer-text-alignment": "left",
										"--framer-text-color":
											"var(--extracted-1ok3k95, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
									}}
								>
									<motion.li data-preset-tag={"p"}>
										<motion.p
											style={{
												"--framer-text-alignment": "left",
												"--framer-text-color":
													"var(--extracted-oa5vkk, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
											}}
										>
											{"1 Medium Buffalo Bliss"}
										</motion.p>
									</motion.li>
								</motion.ul>
							</React.Fragment>
						</RichText>
						<motion.div
							className={"framer-nnwcsm"}
							data-framer-name={"Pizzas Stack"}
							layoutDependency={layoutDependency}
							layoutId={"oGxssdcx5"}
						>
							<motion.div
								className={"framer-1u12prb"}
								layoutDependency={layoutDependency}
								layoutId={"lKLD2qPxR"}
							>
								<ChildrenCanSuspend>
									<QueryData
										query={query1()}
										{...addPropertyOverrides(
											{
												E7X05txrn: {
													query: query9(),
												},
												JMpjGRmKa: {
													query: query7(),
												},
												kxfVpU5z6: {
													query: query9(),
												},
												MMad5lWE1: {
													query: query3(),
												},
												MTxAdEQft: {
													query: query7(),
												},
												MUdLR3PJV: {
													query: query9(),
												},
												noJGfd7Q9: {
													query: query9(),
												},
												OquFlA0G4: {
													query: query5(),
												},
												RxVeqV7K2: {
													query: query5(),
												},
												w3dbh6gAh: {
													query: query3(),
												},
											},
											baseVariant,
											gestureVariant,
										)}
									>
										{(collection, paginationInfo, loadMore) => {
											return (
												<_Fragment>
													{collection?.map(
														(
															{
																id: idlKLD2qPxR2,
																IvnecNHdQ: IvnecNHdQlKLD2qPxR2,
																uS7iNeSm_: uS7iNeSm_lKLD2qPxR2,
															},
															index,
														) => {
															IvnecNHdQlKLD2qPxR2 ??= "";
															return (
																<LayoutGroup id={`lKLD2qPxR-${idlKLD2qPxR2}`}>
																	<PathVariablesContext.Provider
																		value={{
																			IvnecNHdQ: IvnecNHdQlKLD2qPxR2,
																		}}
																	>
																		<motion.div
																			className={"framer-pssqb"}
																			layoutDependency={layoutDependency}
																			layoutId={"TMR8P2UyI"}
																		>
																			<ImageWithFX
																				__framer__spring={{
																					damping: 60,
																					delay: 0,
																					duration: 0.3,
																					ease: [0.44, 0, 0.56, 1],
																					mass: 1,
																					stiffness: 200,
																					type: "spring",
																				}}
																				__framer__styleTransformEffectEnabled={
																					true
																				}
																				__framer__transformTargets={[
																					{
																						target: {
																							opacity: 1,
																							rotate: 0,
																							rotateX: 0,
																							rotateY: 0,
																							scale: 1,
																							skewX: 0,
																							skewY: 0,
																							x: 0,
																							y: 24,
																						},
																					},
																					{
																						target: {
																							opacity: 1,
																							rotate: 15,
																							rotateX: 0,
																							rotateY: 0,
																							scale: 1,
																							skewX: 0,
																							skewY: 0,
																							x: 0,
																							y: 0,
																						},
																					},
																				]}
																				__framer__transformTrigger={"onInView"}
																				__perspectiveFX={false}
																				__smartComponentFX={true}
																				__targetOpacity={1}
																				background={{
																					alt: "",
																					fit: "fill",
																					sizes: "400px",
																					...toResponsiveImage(
																						uS7iNeSm_lKLD2qPxR2,
																					),
																				}}
																				className={"framer-1f29hs6"}
																				data-framer-name={"Photo"}
																				layoutDependency={layoutDependency}
																				layoutId={"GwsVNZVM0"}
																				style={{
																					borderBottomLeftRadius: 10,
																					borderBottomRightRadius: 10,
																					borderTopLeftRadius: 10,
																					borderTopRightRadius: 10,
																				}}
																				{...addPropertyOverrides(
																					{
																						E7X05txrn: {
																							background: {
																								alt: "",
																								fit: "fill",
																								sizes: "720px",
																								...toResponsiveImage(
																									uS7iNeSm_lKLD2qPxR2,
																								),
																							},
																						},
																						noJGfd7Q9: {
																							background: {
																								alt: "",
																								fit: "fill",
																								sizes: "720px",
																								...toResponsiveImage(
																									uS7iNeSm_lKLD2qPxR2,
																								),
																							},
																						},
																					},
																					baseVariant,
																					gestureVariant,
																				)}
																			/>
																		</motion.div>
																	</PathVariablesContext.Provider>
																</LayoutGroup>
															);
														},
													)}
												</_Fragment>
											);
										}}
									</QueryData>
								</ChildrenCanSuspend>
							</motion.div>
							<motion.div
								className={"framer-1y8t7x1"}
								layoutDependency={layoutDependency}
								layoutId={"dihO8NRkz"}
							>
								<ChildrenCanSuspend>
									<QueryData
										query={query11()}
										{...addPropertyOverrides(
											{
												E7X05txrn: {
													query: query19(),
												},
												JMpjGRmKa: {
													query: query17(),
												},
												kxfVpU5z6: {
													query: query19(),
												},
												MMad5lWE1: {
													query: query13(),
												},
												MTxAdEQft: {
													query: query17(),
												},
												MUdLR3PJV: {
													query: query19(),
												},
												noJGfd7Q9: {
													query: query19(),
												},
												OquFlA0G4: {
													query: query15(),
												},
												RxVeqV7K2: {
													query: query15(),
												},
												w3dbh6gAh: {
													query: query13(),
												},
											},
											baseVariant,
											gestureVariant,
										)}
									>
										{(collection1, paginationInfo1, loadMore1) => {
											return (
												<_Fragment>
													{collection1?.map(
														(
															{
																id: iddihO8NRkz2,
																IvnecNHdQ: IvnecNHdQdihO8NRkz2,
																uS7iNeSm_: uS7iNeSm_dihO8NRkz2,
															},
															index1,
														) => {
															IvnecNHdQdihO8NRkz2 ??= "";
															return (
																<LayoutGroup id={`dihO8NRkz-${iddihO8NRkz2}`}>
																	<PathVariablesContext.Provider
																		value={{
																			IvnecNHdQ: IvnecNHdQdihO8NRkz2,
																		}}
																	>
																		<motion.div
																			className={"framer-upi6g7"}
																			layoutDependency={layoutDependency}
																			layoutId={"mwq0uREKM"}
																		>
																			<ImageWithFX
																				__framer__spring={{
																					damping: 60,
																					delay: 0,
																					duration: 0.3,
																					ease: [0.44, 0, 0.56, 1],
																					mass: 1,
																					stiffness: 200,
																					type: "spring",
																				}}
																				__framer__styleTransformEffectEnabled={
																					true
																				}
																				__framer__transformTargets={[
																					{
																						target: {
																							opacity: 1,
																							rotate: 0,
																							rotateX: 0,
																							rotateY: 0,
																							scale: 1,
																							skewX: 0,
																							skewY: 0,
																							x: 0,
																							y: 48,
																						},
																					},
																					{
																						target: {
																							opacity: 1,
																							rotate: 15,
																							rotateX: 0,
																							rotateY: 0,
																							scale: 1,
																							skewX: 0,
																							skewY: 0,
																							x: 0,
																							y: 0,
																						},
																					},
																				]}
																				__framer__transformTrigger={"onInView"}
																				__perspectiveFX={false}
																				__smartComponentFX={true}
																				__targetOpacity={1}
																				background={{
																					alt: "",
																					fit: "fill",
																					sizes: "400px",
																					...toResponsiveImage(
																						uS7iNeSm_dihO8NRkz2,
																					),
																				}}
																				className={"framer-1juzdk4"}
																				data-framer-name={"Photo"}
																				layoutDependency={layoutDependency}
																				layoutId={"Q_eTYKgku"}
																				style={{
																					borderBottomLeftRadius: 10,
																					borderBottomRightRadius: 10,
																					borderTopLeftRadius: 10,
																					borderTopRightRadius: 10,
																				}}
																				{...addPropertyOverrides(
																					{
																						E7X05txrn: {
																							background: {
																								alt: "",
																								fit: "fill",
																								sizes: "720px",
																								...toResponsiveImage(
																									uS7iNeSm_dihO8NRkz2,
																								),
																							},
																						},
																						noJGfd7Q9: {
																							background: {
																								alt: "",
																								fit: "fill",
																								sizes: "720px",
																								...toResponsiveImage(
																									uS7iNeSm_dihO8NRkz2,
																								),
																							},
																						},
																					},
																					baseVariant,
																					gestureVariant,
																				)}
																			/>
																		</motion.div>
																	</PathVariablesContext.Provider>
																</LayoutGroup>
															);
														},
													)}
												</_Fragment>
											);
										}}
									</QueryData>
								</ChildrenCanSuspend>
							</motion.div>
						</motion.div>
						<motion.div
							className={"framer-1svor90"}
							data-framer-name={"Pricing"}
							layoutDependency={layoutDependency}
							layoutId={"vlfXJ232b"}
						>
							<ComponentViewportProvider
								height={36}
								y={
									(componentViewport?.y || 0) +
									48 +
									(((componentViewport?.height || 200) - 288 - 427.2) / 2 +
										331.2 +
										48) +
									12
								}
								{...addPropertyOverrides(
									{
										BgVKR3peo: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 264 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										E7X05txrn: {
											y:
												(componentViewport?.y || 0) +
												48 +
												(((componentViewport?.height || 200) - 488 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										kxfVpU5z6: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 264 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										MTxAdEQft: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 264 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										noJGfd7Q9: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 464 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										OquFlA0G4: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 264 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
										w3dbh6gAh: {
											y:
												(componentViewport?.y || 0) +
												24 +
												(((componentViewport?.height || 200) - 264 - 427.2) /
													2 +
													331.2 +
													48) +
												12,
										},
									},
									baseVariant,
									gestureVariant,
								)}
							>
								<SmartComponentScopedContainer
									className={"framer-cug6of-container"}
									layoutDependency={layoutDependency}
									layoutId={"OVcwoXE0H-container"}
									nodeId={"OVcwoXE0H"}
									rendersWithMotion={true}
									scopeId={"sdO1Bzv_P"}
								>
									{_jsx(stdin_default, {
										height: "100%",
										id: "OVcwoXE0H",
										layoutId: "OVcwoXE0H",
										lJNtxFrg5:
											"https://www.doordash.com/?srsltid=AfmBOooMK5uCbQ6Ohgafa0bm6OYZAhbWRLS-nrNo0prqBqwQkYTXdMDe",
										O5tlPGRu3: "Order Now",
										variant: matchVariant("Te0yMoM0l"),
										width: "100%",
									})}
								</SmartComponentScopedContainer>
							</ComponentViewportProvider>
							<motion.div
								className={"framer-1d6xvs4"}
								data-framer-name={"Pricing"}
								layoutDependency={layoutDependency}
								layoutId={"FxCxzpQqM"}
							>
								<RichText
									__fromCanvasComponent={true}
									className={"framer-1ramlp8"}
									data-framer-name={"Price - Medium"}
									fonts={["Inter"]}
									layoutDependency={layoutDependency}
									layoutId={"ykZN1pAYk"}
									style={{
										"--extracted-r6o4lv":
											"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
									}}
									variants={{
										MMad5lWE1: {
											"--extracted-r6o4lv":
												"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
										},
										w3dbh6gAh: {
											"--extracted-r6o4lv":
												"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
										},
									}}
									verticalAlignment={"top"}
									withExternalLayout={true}
									{...addPropertyOverrides(
										{
											E7X05txrn: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$23.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											JMpjGRmKa: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$22.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											kxfVpU5z6: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$23.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											MMad5lWE1: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"$22.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											MTxAdEQft: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$22.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											MUdLR3PJV: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$23.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											noJGfd7Q9: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
															}}
														>
															{"$23.99"}
														</motion.p>
													</React.Fragment>
												),
											},
											w3dbh6gAh: {
												children: (
													<React.Fragment>
														<motion.p
															className={"framer-styles-preset-18kbsqc"}
															data-styles-preset={"e4uTd5msu"}
															style={{
																"--framer-text-color":
																	"var(--extracted-r6o4lv, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
															}}
														>
															{"$22.99"}
														</motion.p>
													</React.Fragment>
												),
											},
										},
										baseVariant,
										gestureVariant,
									)}
								>
									<React.Fragment>
										<motion.p
											className={"framer-styles-preset-18kbsqc"}
											data-styles-preset={"e4uTd5msu"}
											dir={"auto"}
											style={{
												"--framer-text-color":
													"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
											}}
										>
											{"$21.99"}
										</motion.p>
									</React.Fragment>
								</RichText>
								<motion.div
									className={"framer-1lg3ku0"}
									data-framer-name={"Saving"}
									layoutDependency={layoutDependency}
									layoutId={"HXDXGIffG"}
								>
									<RichText
										__fromCanvasComponent={true}
										className={"framer-sr5x4k"}
										data-framer-name={"Price - Medium"}
										fonts={["Inter-Light"]}
										layoutDependency={layoutDependency}
										layoutId={"Tnum3aQA7"}
										style={{
											"--extracted-r6o4lv":
												"var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255))",
										}}
										variants={{
											MMad5lWE1: {
												"--extracted-r6o4lv":
													"var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51))",
											},
										}}
										verticalAlignment={"top"}
										withExternalLayout={true}
										{...addPropertyOverrides(
											{
												BgVKR3peo: {
													children: (
														<React.Fragment>
															<motion.p
																dir={"auto"}
																style={{
																	"--framer-font-size": "14px",
																	"--framer-line-height": "1.4em",
																	"--framer-text-alignment": "center",
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $4"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
												E7X05txrn: {
													children: (
														<React.Fragment>
															<motion.p
																style={{
																	"--font-selector": "SW50ZXItTGlnaHQ=",
																	"--framer-font-family":
																		'"Inter", "Inter Placeholder", sans-serif',
																	"--framer-font-size": "22px",
																	"--framer-font-weight": "300",
																	"--framer-text-alignment": "center",
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $6"}
															</motion.p>
														</React.Fragment>
													),
												},
												JMpjGRmKa: {
													children: (
														<React.Fragment>
															<motion.p
																style={{
																	"--font-selector": "SW50ZXItTGlnaHQ=",
																	"--framer-font-family":
																		'"Inter", "Inter Placeholder", sans-serif',
																	"--framer-font-size": "22px",
																	"--framer-font-weight": "300",
																	"--framer-text-alignment": "center",
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $5"}
															</motion.p>
														</React.Fragment>
													),
												},
												kxfVpU5z6: {
													children: (
														<React.Fragment>
															<motion.p
																className={"framer-styles-preset-146xxk6"}
																data-styles-preset={"OIpMKy3L7"}
																style={{
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $6"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
												MMad5lWE1: {
													children: (
														<React.Fragment>
															<motion.p
																style={{
																	"--font-selector": "SW50ZXItTGlnaHQ=",
																	"--framer-font-family":
																		'"Inter", "Inter Placeholder", sans-serif',
																	"--framer-font-size": "22px",
																	"--framer-font-weight": "300",
																	"--framer-text-alignment": "center",
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-87e6d356-f892-4c3e-bd6c-68461b752930, rgb(51, 51, 51)))",
																}}
															>
																{"- Save $5"}
															</motion.p>
														</React.Fragment>
													),
												},
												MTxAdEQft: {
													children: (
														<React.Fragment>
															<motion.p
																className={"framer-styles-preset-146xxk6"}
																data-styles-preset={"OIpMKy3L7"}
																style={{
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $5"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
												MUdLR3PJV: {
													children: (
														<React.Fragment>
															<motion.p
																style={{
																	"--font-selector": "SW50ZXItTGlnaHQ=",
																	"--framer-font-family":
																		'"Inter", "Inter Placeholder", sans-serif',
																	"--framer-font-size": "22px",
																	"--framer-font-weight": "300",
																	"--framer-text-alignment": "center",
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $6"}
															</motion.p>
														</React.Fragment>
													),
												},
												noJGfd7Q9: {
													children: (
														<React.Fragment>
															<motion.p
																className={"framer-styles-preset-146xxk6"}
																data-styles-preset={"OIpMKy3L7"}
																style={{
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $6"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
												OquFlA0G4: {
													children: (
														<React.Fragment>
															<motion.p
																className={"framer-styles-preset-146xxk6"}
																data-styles-preset={"OIpMKy3L7"}
																dir={"auto"}
																style={{
																	"--framer-text-color":
																		"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
																}}
															>
																{"- Save $4"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
												w3dbh6gAh: {
													children: (
														<React.Fragment>
															<motion.p
																className={"framer-styles-preset-146xxk6"}
																data-styles-preset={"OIpMKy3L7"}
															>
																{"- Save $5"}
															</motion.p>
														</React.Fragment>
													),
													fonts: ["Inter"],
												},
											},
											baseVariant,
											gestureVariant,
										)}
									>
										<React.Fragment>
											<motion.p
												dir={"auto"}
												style={{
													"--font-selector": "SW50ZXItTGlnaHQ=",
													"--framer-font-size": "22px",
													"--framer-font-weight": "300",
													"--framer-text-alignment": "center",
													"--framer-text-color":
														"var(--extracted-r6o4lv, var(--token-c53bc032-a445-44bf-b542-b566e1c9486a, rgb(255, 255, 255)))",
												}}
											>
												{"- Save $4"}
											</motion.p>
										</React.Fragment>
									</RichText>
								</motion.div>
							</motion.div>
						</motion.div>
					</motion.div>
				</Transition>
			</Variants>
		</LayoutGroup>
	);
});
var css5 = [
	"@supports (aspect-ratio: 1) { body { --framer-aspect-ratio-supported: auto; } }",
	".framer-odiKA.framer-jm8wpu, .framer-odiKA .framer-jm8wpu { display: block; }",
	".framer-odiKA.framer-xcd8td { align-content: flex-start; align-items: flex-start; display: flex; flex-direction: column; flex-wrap: nowrap; gap: 24px; height: min-content; justify-content: center; overflow: hidden; padding: 48px 48px 240px 48px; position: relative; width: 600px; will-change: var(--framer-will-change-override, transform); }",
	".framer-odiKA .framer-1wrnumo, .framer-odiKA .framer-sr5x4k { flex: none; height: auto; position: relative; white-space: pre; width: auto; z-index: 1; }",
	".framer-odiKA .framer-1jg7xgw { flex: none; height: auto; position: relative; white-space: pre-wrap; width: 468px; word-break: break-word; word-wrap: break-word; z-index: 1; }",
	".framer-odiKA .framer-nnwcsm { align-content: center; align-items: center; bottom: 0px; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 10px; height: 216px; justify-content: center; left: 0px; overflow: visible; padding: 0px; position: absolute; right: 0px; z-index: 1; }",
	".framer-odiKA .framer-1u12prb { align-content: center; align-items: center; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 40px; height: min-content; justify-content: flex-start; left: -48px; padding: 0px; position: absolute; top: 0px; width: min-content; z-index: 1; }",
	".framer-odiKA .framer-pssqb, .framer-odiKA .framer-upi6g7 { align-content: center; align-items: center; display: flex; flex: none; flex-direction: row; flex-wrap: wrap; gap: 40px; height: min-content; justify-content: flex-start; padding: 0px; position: relative; width: 400px; }",
	".framer-odiKA .framer-1f29hs6, .framer-odiKA .framer-1juzdk4 { aspect-ratio: 1 / 1; flex: 1 0 0px; height: var(--framer-aspect-ratio-supported, 200px); position: relative; width: 1px; }",
	".framer-odiKA .framer-1y8t7x1 { align-content: center; align-items: center; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 40px; height: min-content; justify-content: flex-start; padding: 0px; position: absolute; right: -48px; top: 0px; width: min-content; z-index: 1; }",
	".framer-odiKA .framer-1svor90 { align-content: center; align-items: center; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; height: min-content; justify-content: space-between; overflow: visible; padding: 12px 0px 0px 0px; position: relative; width: 100%; z-index: 1; }",
	".framer-odiKA .framer-cug6of-container { flex: none; height: auto; position: relative; width: auto; }",
	".framer-odiKA .framer-1d6xvs4 { align-content: flex-end; align-items: flex-end; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 8px; height: min-content; justify-content: center; overflow: visible; padding: 0px; position: relative; width: min-content; }",
	".framer-odiKA .framer-1ramlp8 { flex: none; height: auto; position: relative; white-space: pre; width: auto; }",
	".framer-odiKA .framer-1lg3ku0 { align-content: center; align-items: center; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 10px; height: min-content; justify-content: center; overflow: visible; padding: 0px; position: relative; width: min-content; }",
	".framer-odiKA.framer-v-106v0r8.framer-xcd8td { padding: 48px 48px 440px 48px; width: 1200px; }",
	".framer-odiKA.framer-v-106v0r8 .framer-nnwcsm, .framer-odiKA.framer-v-1m3c57k .framer-nnwcsm { height: 456px; }",
	".framer-odiKA.framer-v-106v0r8 .framer-pssqb, .framer-odiKA.framer-v-106v0r8 .framer-upi6g7, .framer-odiKA.framer-v-1m3c57k .framer-pssqb, .framer-odiKA.framer-v-1m3c57k .framer-upi6g7 { width: 720px; }",
	".framer-odiKA.framer-v-1uic2k7.framer-xcd8td, .framer-odiKA.framer-v-4u0o7z.framer-xcd8td, .framer-odiKA.framer-v-1wr12ct.framer-xcd8td, .framer-odiKA.framer-v-7sm5nt.framer-xcd8td, .framer-odiKA.framer-v-1o1vf8b.framer-xcd8td { padding: 24px 24px 240px 24px; width: 358px; }",
	".framer-odiKA.framer-v-1uic2k7 .framer-1u12prb, .framer-odiKA.framer-v-4u0o7z .framer-1u12prb, .framer-odiKA.framer-v-1wr12ct .framer-1u12prb, .framer-odiKA.framer-v-7sm5nt .framer-1u12prb, .framer-odiKA.framer-v-1o1vf8b .framer-1u12prb, .framer-odiKA.framer-v-1m3c57k .framer-1u12prb { left: -120px; }",
	".framer-odiKA.framer-v-1uic2k7 .framer-1y8t7x1, .framer-odiKA.framer-v-4u0o7z .framer-1y8t7x1, .framer-odiKA.framer-v-1wr12ct .framer-1y8t7x1, .framer-odiKA.framer-v-7sm5nt .framer-1y8t7x1, .framer-odiKA.framer-v-1o1vf8b .framer-1y8t7x1, .framer-odiKA.framer-v-1m3c57k .framer-1y8t7x1 { right: -120px; }",
	".framer-odiKA.framer-v-1uic2k7 .framer-1lg3ku0, .framer-odiKA.framer-v-4u0o7z .framer-1lg3ku0, .framer-odiKA.framer-v-1wr12ct .framer-1lg3ku0, .framer-odiKA.framer-v-7sm5nt .framer-1lg3ku0, .framer-odiKA.framer-v-1o1vf8b .framer-1lg3ku0, .framer-odiKA.framer-v-1m3c57k .framer-1lg3ku0 { padding: 0px 0px 1px 0px; }",
	".framer-odiKA.framer-v-1m3c57k.framer-xcd8td { padding: 24px 24px 440px 24px; width: 762px; }",
	...css3,
	...css4,
	...css,
	...css2,
];
var FramersdO1Bzv_P = withCSS(Component, css5, "framer-odiKA");
var stdin_default3 = FramersdO1Bzv_P;
FramersdO1Bzv_P.displayName = "Menu Products/Offer Crad";
FramersdO1Bzv_P.defaultProps = {
	height: 491,
	width: 600,
};
addPropertyControls(FramersdO1Bzv_P, {
	variant: {
		options: [
			"JlqgQhEnq",
			"MMad5lWE1",
			"RxVeqV7K2",
			"JMpjGRmKa",
			"MUdLR3PJV",
			"E7X05txrn",
			"BgVKR3peo",
			"w3dbh6gAh",
			"OquFlA0G4",
			"MTxAdEQft",
			"kxfVpU5z6",
			"noJGfd7Q9",
		],
		optionTitles: [
			"Spicy Duo Deal",
			"Cheese Lovers Pair",
			"Veggie Delight Duo",
			"Sweet & Savory Combo",
			"Meat Feast Combo",
			"Meat Feast Combo - Big",
			"Spicy Duo Deal - Mobile",
			"Cheese Lovers Pair - Mobile",
			"Veggie Delight Duo - Mobile",
			"Sweet & Savory Combo - Mobile",
			"Meat Feast Combo - Mobile",
			"Meat Feast Combo - Mobile - Tablet",
		],
		title: "Variant",
		type: ControlType.Enum,
	},
});
addFonts(
	FramersdO1Bzv_P,
	[
		{
			explicitInter: true,
			fonts: [
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
					url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
					url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+1F00-1FFF",
					url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+0370-03FF",
					url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
					url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
					url: "https://framerusercontent.com/assets/GrgcKwrN6d3Uz8EwcLHZxwEfC4.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
					url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
					weight: "400",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
					url: "https://framerusercontent.com/assets/BkDpl4ghaqvMi1btKFyG2tdbec.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
					url: "https://framerusercontent.com/assets/zAMK70AQRFSShJgUiaR5IiIhgzk.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+1F00-1FFF",
					url: "https://framerusercontent.com/assets/IETjvc5qzUaRoaruDpPSwCUM8.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange: "U+0370-03FF",
					url: "https://framerusercontent.com/assets/oLCoaT3ioA0fHdJnWR9W6k7NY.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
					url: "https://framerusercontent.com/assets/Sj0PCHQSBjFmEp6NBWg6FNaKc.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
					url: "https://framerusercontent.com/assets/aqiiD4LUKkKzXdjGL5UzHq8bo5w.woff2",
					weight: "300",
				},
				{
					cssFamilyName: "Inter",
					source: "framer",
					style: "normal",
					uiFamilyName: "Inter",
					unicodeRange:
						"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
					url: "https://framerusercontent.com/assets/H4TfENUY1rh8R9UaSD6vngjJP3M.woff2",
					weight: "300",
				},
			],
		},
		...ButtonFonts,
		...getFontsFromSharedStyle(fonts3),
		...getFontsFromSharedStyle(fonts4),
		...getFontsFromSharedStyle(fonts),
		...getFontsFromSharedStyle(fonts2),
	],
	{
		supportsExplicitInterCodegen: true,
	},
);
FramersdO1Bzv_P.loader = {
	load: (props, context) => {
		const locale = context.locale;
		const queryCacheEntry = queryCache.get(query1(), locale);
		const queryCacheEntry1 = queryCache.get(query3(), locale);
		const queryCacheEntry2 = queryCache.get(query5(), locale);
		const queryCacheEntry3 = queryCache.get(query7(), locale);
		const queryCacheEntry4 = queryCache.get(query9(), locale);
		const queryCacheEntry5 = queryCache.get(query11(), locale);
		const queryCacheEntry6 = queryCache.get(query13(), locale);
		const queryCacheEntry7 = queryCache.get(query15(), locale);
		const queryCacheEntry8 = queryCache.get(query17(), locale);
		const queryCacheEntry9 = queryCache.get(query19(), locale);
		return Promise.allSettled([
			queryCacheEntry.preload(),
			queryCacheEntry1.preload(),
			queryCacheEntry2.preload(),
			queryCacheEntry3.preload(),
			queryCacheEntry4.preload(),
			queryCacheEntry5.preload(),
			queryCacheEntry6.preload(),
			queryCacheEntry7.preload(),
			queryCacheEntry8.preload(),
			queryCacheEntry9.preload(),
			forwardLoader(stdin_default, {}, context),
		]);
	},
};

// virtual:menu-products/offer-crad
import { WithFramerBreakpoints } from "unframer";
import { jsx } from "react/jsx-runtime";
var locales = [];
var defaultResponsiveVariants = {};
/** @type {function(Props): any} */
function ComponentWithRoot({ locale, ...rest }) {
	return (
		<ContextProviders
			routes={routes}
			framerSiteId={
				"a377177818209678b37adc5f263446ba17148664a97f412372f73ef06885d900"
			}
			locale={locale}
			locales={locales}
		>
			{jsx(stdin_default3, {
				...rest,
			})}
		</ContextProviders>
	);
}
/**
 * @type {import("unframer").UnframerBreakpoint}
 * Represents a responsive breakpoint for unframer.
 */

/**
 * @typedef VariantsMap
 * Partial record of UnframerBreakpoint to Props.variant, with a mandatory 'base' key.
 * { [key in UnframerBreakpoint]?: Props['variant'] } & { base: Props['variant'] }
 */

/**
 * Renders OfferCradFramerComponent for all breakpoints with a variants map. Variant prop is inferred per breakpoint.
 * @function
 * @param {Omit<Props, 'variant'> & {variants?: VariantsMap}} props
 * @returns {any}
 */
ComponentWithRoot.Responsive = ({ locale = "", ...rest }) => {
	return (
		<ContextProviders
			routes={routes}
			framerSiteId={
				"a377177818209678b37adc5f263446ba17148664a97f412372f73ef06885d900"
			}
			locale={locale}
			locales={locales}
		>
			<WithFramerBreakpoints
				Component={stdin_default3}
				variants={defaultResponsiveVariants}
				{...rest}
			/>
		</ContextProviders>
	);
};
Object.assign(ComponentWithRoot, stdin_default3);
var offer_crad_default = ComponentWithRoot;
export { offer_crad_default as default };
